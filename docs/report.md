...
- CI green + 감시관 §5 7항목 통과 + 브라우저 실측 완전 일치까지 확인했다. AGENTS §3에
  따라 **최종 `[x]` 확정은 보리의 명시 승인**이 필요 — 아직 승인 전이라 `[~]` 유지.
- 승인 시 남은 건 콜상세 폼(call-detail-form.css) 1개 슬라이스뿐 — 그게 끝나면
  §3 설계표 전체 완료(`main-calendar.css` 해체).
