...
| 7 요구사항 | 전 화면 공백 없음 통일 — 브라우저 실검증 중 1건 누락 발견·수정(아래) |

### 추가 수정 (`383698d`) — 브라우저 검증 중 보리 발견

매출 화면 "운송 수입"/"운행 지출" 카드의 "합계" 행 2곳
(`OwnerMonthlyCards.jsx:105,138`)이 통일 대상에서 빠져 있었음 — 이
두 줄만 `formatWon()`/`won()` 헬퍼를 안 쓰고 자체 인라인
`.toLocaleString('ko-KR')} 원`을 써서 애초에 14개 파일 grep에 안
걸렸던 것. 같은 파일의 VAT 행(`won(detail.vatAmount)`) 등은 이미
`won()` 헬퍼(공백 없음)를 쓰고 있어 문제 없었음. 공백 문자만 제거,
로직 변경 없음. 로컬 test 163·typecheck 통과. 전체 `*.jsx` 재검색
(toLocaleString+원 패턴, "원" 앞 공백)으로 다른 누락 없음 확인.

### 검증 방법 (브라우저)

미수금·리포트·세금계산서·정비유류기타·기사정산·캘린더·**매출(수입/
지출 합계 포함)** 등 금액이 보이는 화면 전반에서 "OOO원"(공백 없음)
으로 통일됐는지 확인. 라이트/다크 무관(텍스트만 바뀜, 레이아웃 영향
없음).
