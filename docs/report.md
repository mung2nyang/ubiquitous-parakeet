...
## `side-menu.css` 분리설계안 (보리 지시, 2026-09-15) — 슬라이스 G `[~]`
...
1. `components/report/report.css` 신규 — `side-menu.css:800-1032`(233줄)
   `.report-*`/`info-table`/PDF출력모드 규칙 이동, 값 변경 없음.
2. import 추가: `ReportPage`·`ReportDetailView`·`ReportShareModal`·
   `ReportSummaryContent`.
3. `side-menu.css` 1421→1188줄. `.report-page-wrap` 페이지 패딩(공용
   선택자 목록)은 그대로 유지.
...
| 2 몰래 증설 | 없음(순수 이동) |
| 3 타입 꼼수 | 없음 |
| 4 200줄 | report.css 233(화면 전용). side-menu 1188(여전히 초과·후속) |
| 5 테스트 | 163 통과 |
| 7 요구사항 | G 분리 완료 — 브라우저 실검증 대기 |

### 검증 방법 (브라우저)

운송비 내역서 — 요약/세부 표·PDF·공유 모달·파렛트 2열 분할·다크/라이트
분리 전과 동일한지.
