...
- **테스트 진실성(§5-5)**: 기존 두 기대값에 `driverName: ''` 보강(약화
  ...
  약화 없음.
- **감시관 실측(제한적)**: `npm run dev`로 로컬 접속했으나 열린 세션이
  보리의 **실계정(로그인된 실데이터, 등록 차량 0대)**이었음 — 등록 차량이
  없어 15-5가 요구하는 4개 시나리오(기사명 유/무 사이드메뉴·관리화면
  제목, 연동 기사 회귀, `notFound`)를 게스트 테스트 데이터로 재현하지
  못했다. 실데이터에 테스트용 차량을 추가하는 건 임의 데이터 조작이라
  하지 않았다(읽기 탐색만 하고 원복 없이 뒤로가기). **따라서 §5-7
  요구사항 충족 항목은 diff/설계 대조로만 확인했고, 브라우저 실측은
  미완료** — `[x]` 확정 전 브라우저 실측(게스트 계정으로) 한 번 더
  필요.
- **AGENTS §5 판정**: 1~6항목 통과(범위·증설없음·타입·200줄·테스트
  진실성·문서정합). 7항목(요구사항 충족)은 코드 대조로는 설계와 일치
  확인했으나 **브라우저 실측 미완**이라 완전 통과로 못 박지 않음.
- **미승인 상태 유지**: 위 사유로 `537cd48`은 `[~]`. 게스트 계정으로 4개
  시나리오 실측 후 §5 7항목 완전 통과 확인 + 보리 최종 승인이 남았다.
