...
| 미연동 기사 거래처 | 차주 계정 안에 `scopedToVehicleNumber` 태그로 저장(로그인이 없어 차주 데이터에 얹음) | 원본도 **의도적으로 숨김**(`driver-link.js:932-935` 주석 — 차주 것과 저장공간이 겹치므로) |
...
**미연동은 원본 그대로 맞음** — 고정노선 숨김은 버그가 아니라 원본 설계.
