...
## 4-4c. 2026-09-17 — 고정노선 거래처 1곳 제한 (현재 구조, 버그 아님) + 추후 구상

고정노선은 차주 전체 거래처 목록 중 **한 곳만** 연결 가능하다. 그 거래처의
`fixedUnitPrice` 하나만 쓰이며, 노선마다 다른 단가를 여러 개 두는 건 지금 구조에서
지원하지 않는다. 단가 금액 자체를 바꾸는 건 지금처럼 가능(거래처 화면에서 수정).

**추후 검토 중(미승인, 이관 완료 후):** 거래처별로 고정노선을 여러 개 등록할 수 있게
확장하는 방안을 보리가 구상 중. 착수 전 별도 지시·승인 필요.
