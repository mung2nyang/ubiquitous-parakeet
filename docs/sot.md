...
로그인 업무 데이터의 정본을 Supabase만으로 옮기는 Fail-Fast는 A~D `[x]`. E(로그인 LS 미러 + 남은 저장은 서버 성공 후 Store)는 `docs/archive/slice-e.md`, 구현 `[~]`(보리 실검증·`[x]` 전).

## 6. 다음 세션 체크리스트

1. `AGENTS.md` §0 + §0-1 읽기.
2. 슬라이스 E는 `docs/archive/slice-e.md`. 구현 `[~]`. `[x]`는 보리 브라우저 확인 후. D는 `[x]`.
3. 신규 큐/overlay 넣지 않기. 로그인 저장 실패 토스트: `저장에 실패했습니다. 네트워크 상태를 확인해 주세요.`
4. **푸시하지 말 것**(별도 지시 전).
5. **게스트 기사·차량 초대를 고치지 마라** (아래 8절).
