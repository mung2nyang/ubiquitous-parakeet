...
설정 화면엔 기사 계산서 기준을 "공제 전/후" 중에서 고르는 토글이 있지만, 공통 계산 로직에
실제로 넘어가는 값은 지금 **공제후로 고정**돼 있다. 화면 선택이 계산에 반영 안 되는 것처럼
보이지만 **버그가 아니라 의도된 정리 과정**이다.

이유: "공제 전/후" 개념은 원본(바닐라) 앱에 있던 것인데, 실제 현직 화물기사들은 "공제전"을
쓰지 않고 전부 "공제후"만 쓴다(보리 확인). 그래서 이관 과정에서 계산 로직을 먼저 공제후로
고정해 정리 중이고, **이관 완료 시 "공제 전" 선택지 자체를 화면에서 없앨 예정**이다(과도기라
지금은 화면에 토글만 남아있음). "원본에 있는 옵션 = 지금도 실제로 쓰는 개념"이 아닌
사례 중 하나이니 주의.
