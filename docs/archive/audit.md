...
## 5-11. Fail-Fast 슬라이스 D — 로그인 메인 일지 + hydrate 빈 daily_logs (2026-09-01)

로그인 메인(서버에 `supabaseId` 있는 차량) 일지: `commitMainDayLogToCloud`가 그 날짜 `daily_logs` upsert 또는 빈 날 delete 1회. 성공 후에만 `commitWorkData(..., { syncToCloud: false })`. 실패 토스트 A~C와 동일. 신규 durable/tombstone 없음. `pendingWorkDataWrites.js`는 게스트·미동기화 메인용으로 남김. 로그인+동기화 메인은 재시도 리스너 비활성.
...
## 원본↔React UI 전수 대조 — 1차 조사 (2026-09-07, 감시관 직접 조사, 코드 수정 없음)

보리 지시(2026-09-07): 전체 화면 한번에, 감시관이 로컬로 직접 켜서(정적 서버 8791 +
vite dev 5173), 보리 테스트계정으로 로그인한 상태에서 대조. **이 조사는 차이 발견·
기록까지만, 코드 수정·완료판정 없음.** 아래는 당시 `docs/report.md` 전체를 그대로
보존한 것(다음 슬라이스 착수지시서로 리셋됨) — 화면별 §번호는 이후 슬라이스들에서도
계속 참조되니 지우지 않는다.
...
**개별 승인 기록이 있는 것 — 딱 1건**: 차주/소속기사 뱃지 삭제(§2, STATUS.md
"Step 9 ② 1차"에 보리 `[x]` 기록 있음).

**"이관 후 UI 정리" 대상으로 재분류 — 개별 승인 아니었음(2026-09-07 보리 정정)**:
테마 저장 위치(로컬↔클라우드, §1), 서브차량 설정 위치 통합(§2), 차량관리 "회사정산"
라벨 대체(§6), 기사연동관리 폼/목록 UI 패턴 차이(§11). 초반에 "이관 먼저 끝내고
UI는 나중에 한번에 정리하자"고 정해뒀는데 그 결정이 문서에 기록이 안 남아 있었음
— 감시관이 처음엔 "승인된 의도적 변경"이라고 잘못 단정했다가 보리가 바로잡음. 버그도
승인된 변경도 아니고, 이관 완료 후 정리 단계에서 "어느 걸 남길지" 다시 결정할 항목.

**완전히 일치 확인된 화면**: 일일운행(§3), 매출-차주탭(§4), 정비/주유/기타(§7),
세금계산서(§8), 개인정보, 고객센터.

**이번 조사에서 다루지 않음**: 로그인/온보딩 화면 세부(첫 진입 화면만 확인), 기사
연동 상세(`linkedDriverManagementPage`)·연동기사 전용 거래처
...
- **테마 기본값이 다르게 보인 건 이번 테스트 환경 탓(포트가 달라 localStorage가
  분리됨) — 화면 자체의 차이는 아님**: 원본은 `theme`을 localStorage에만 저장하고
  Supabase엔 안 올림(`script.js:2333` 주석 — "다크/라이트 모드는 예전엔 이 로컬 'theme'
  키에만 저장돼서 Supabase에 전혀 올라가지..."). 두 앱이 다른 포트(오리진)라 localStorage가
  분리돼 있어 이번 조사에서 테마가 다르게 보인 것뿐. 다만 저장 위치 자체(로컬 vs
  클라우드)는 react-app이 실제로 다르게 만들어져 있음 — 아래 참고, "이관 후 UI 정리"
  대상으로 재분류.
- **9월 6일 칩 누락 [실제 차이, 원인 특정됨]**: 원본 캘린더 6일 칸엔 "2만" 칩이 있는데
  react-app은 없음. `정비/주유/기타` 화면(§7)에서 대조해보니 데이터 자체는 react-app에
...
- **테마 저장 방식 자체가 다름 [실제 차이, "이관 후 UI 정리" 대상]**: react-app은
  `AppSettingsPage.jsx:120-131`에서 테마를 `useOwnerSettings`(클라우드 동기화되는
  `FinanceSettings.theme`)로 관리 — 기기를 바꿔도 같은 계정이면 테마가 따라옴. 원본은
  `localStorage.getItem('theme')`만 써서 기기별로 따로 논다. 개별 승인 기록은 없음 —
  "이관 먼저, UI 정리는 나중에"라는 초반 방침에 따라 일단 다르게 구현된 상태로 남아있는
  것으로 보임. 이관 완료 후 어느 방식으로 통일할지 결정 필요.
...
- **기사차량(서브차량) 운행일지 설정 — 항목은 다 있음, 화면 배치만 다름 ["이관 후
  UI 정리" 대상]**: 원본은 별도 페이지 `subCarSettingsPage`(`index.html:1389~`)로
  분리돼 있는데, react-app은 `AppSettingsPage.jsx:182-185`에서 "기사차량 운행 일지
  설정" 섹션으로 같은 화면 안에 통합돼 있음. 항목(달력 표시방식·고정노선 등) 자체는
  `FixedRouteBlock` 재사용으로 존재 — 기능 누락은 아니고, 페이지를 나눌지 합칠지는
  UI 정리 단계에서 결정할 사안.
...
- **"차주" 탭은 완전히 일치**: 당월 순이익 -120,000원 / 총 9회 운행 / 부가세
...
- **"전체 손익"·"기사" 탭은 숫자가 서로 다름 [중요, 우선 확인 필요]**:
  ...
  - 원인 후보(코드 근거 확인함): "기사 급여"/월급제 계산은 STATUS.md에 이미 기록된
    ...
    이 개념 자체가 없음) — 그 자체는 의도된 새 방향 기능이라 "이관 누락"은 아님.
  - 그런데 운행 횟수(11↔10, 2↔1)·미입금 건수(0↔1)까지 서로 다른 건 새 기능 유무만으로는
    설명이 안 됨 — 같은 계정·같은 달인데 원본과 react-app이 "몇 건을 원본데이터로
    보는지" 자체가 갈리는 것으로 보임. 재무 숫자라 임의로 넘기지 말고 보리가 최우선으로
    확인해야 할 항목. 코드 수정은 하지 않음.
...
— ["이관 후 UI 정리" 대상]: "회사 정산" 라벨은 매출제/월급제 전환(Step 9) 이전 옛
개념이고 react-app은 그 UI를 이미 지운 상태(메모리
`react-app-driver-settlement-mode-taxonomy` 참고, Step 9는 실제 승인된 슬라이스).
다만 "카드에 뭘 보여줄지" 자체를 개별 승인받은 기록은 없음 — 정보 손실은 아니라
급하지 않지만, UI 정리 때 같이 정리 대상.
...
"연습운수"(사업자 -, 연락처 -)가 하나 더 있음(총 3건). 같은 계정인데 원본이 이
거래처를 아예 조회 못 하는 것으로 보임.

#### 6-2. 미수금/정산 관리 — `receivablesManagementPage` ↔ `/app/receivables`

**원본 "미수금 내역이 없습니다" ↔ react-app엔 미수금 2건 [중요, 우선 확인 필요]**:
react-app엔 "연습운수 2026년8월 운행분 메인차량 미수금 100,000원 1건"·"미지정
거래처 2026년9월 운행분 2222 미수금 200,000원 1건" 2건이 뜨는데, 원본은 이 화면에서
미수금이 하나도 없다고 나옴. 첫 번째 건이 위 §6-1 "연습운수"와 같은 거래처라 한
뿌리로 보이고, 두 번째 건의 "2222"는 기사차량 "22나2222"(운행일지, 미연동)를
가리키는 것으로 추정 — §4(매출 탭)의 "미입금 운송료(1건) 200,000원"과 금액이 정확히
일치해 같은 항목으로 보임. 잠정 결론(코드 확인 전, 가설): 서브차량(특히 22나2222처럼
기사 미연동인 차량)이 만든 운행/거래처 데이터를 react-app은 정상적으로 조회·집계하는데,
원본이 이런 서브차량 관련 데이터를 거래처·미수금·매출 화면 여러 곳에서 일관되게 못
읽고 있는 것으로 의심됨. 반대로 "어느 쪽이 정답 집계인지"는 이 조사만으로 단정 못 함.
...
"홈 캘린더에 9/6 칩이 안 보인다"고 한 그 데이터가 여기선 정상 조회됨 — 그래서 §1
항목을 "캘린더 렌더링 로직 한정 버그"로 원인 범위를 좁혀 놓음.
...
**"월간 운송료 정산" 위젯이 홈 화면과 똑같은 구조 차이를 그대로 반복함 [§1과 동일
원인 추정]**:
...
- react-app: 날짜별 표 없이 "횟수 7회·세부입력 2건 / 1회 단가 50,000원 / 기본운송료
  ...
  내역서에 섞여 들어감).
- §1(홈)과 정확히 같은 패턴이라 같은 계산/표시 컴포넌트를 공유하는 것으로 보이고,
  원인도 하나일 가능성이 높음 — 고칠 때 §1과 §9을 같이 봐야 함.
- "세부 내역서"(거래처별 상세) 버튼은 react-app에 있음(2026-09-07 이관 완료 슬라이스,
  STATUS.md 기록) — 이번 조사에서 열어보진 않음.

#### 10. 개인정보 — `personalInfoPage` ↔ `/app/me/profile`

일치. 사업자명(테스트상호)·사업자번호(111-11-11111)·사업장주소(테스트주소)·업태
(운수업)·종목(화물) 등 항목 동일. react-app은 4개 섹션(01 사업자정보/02 대표자·연락처/
03 정산계좌/04 계정)으로 번호를 매겨 보여주는 배치 차이만 있음.

#### 11. 기사 연동 관리 — `driverConnectionManagementPage` ↔ `/app/drivers`

**통계는 일치, UI 패턴은 다름 ["이관 후 UI 정리" 대상]**: "연동 중 0 / 초대 대기
1"은 두 앱 다 동일. 원본은 초대 입력 폼(기사이름·전화번호·초대코드·할당차량·기간)이
화면에 항상 펼쳐져 있고 코드값이 "929459"로 보임. react-app은 목록형 카드("초대
대기·코드 323630·11가1111·2026-09-07·종료일 없음, 수정/삭제")로 보여주고 새 초대는
"+ 초대" 버튼으로 열게 돼 있음. 코드값(929459 vs 323630)이 서로 다른 게 실제 저장된
값 차이인지, 원본이 폼을 열 때마다 새 코드를 미리 생성해 보여주는 것뿐인지 구분 안
됨 — 화면 패턴 자체가 다르므로 직접 비교가 어려움.
...
첫 진입 화면(로그인/회원가입/비회원으로 시작하기 버튼 3개)만 확인 — 일치. 로그인·
회원가입 폼 내부, 온보딩 마법사는 이번 조사에서 열어보지 않음.

### 이관 계획 (보리 승인, 2026-09-07 — "그 순서로 진행해")

| 순서 | 항목 | 이유 |
|---|---|---|
| 1 | 공지사항 이관(§2) | 정적 목록 3건, 가장 단순한 스코프, 독립적 |
| 2 | 문자 문구 설정 이관(§2) | §1보다 크지만(문구 4종 저장+불러오기) 여전히 독립적 |
| 3 | 홈 캘린더 9/6 칩 렌더링 버그(§1) | 데이터 조회는 정상 확인됨(§7) — 렌더링 로직만 고치면 될 듯 |
| 4 | "월간 운송료 정산" 위젯 구조 통일(§1·§9) | 원본(거래처별) vs react(횟수·단가별) 중 어느 형식으로 갈지 보리 결정 먼저 필요 |
| 5 | 매출 탭 수치 불일치 + 거래처/미수금 데이터 차이(§4·§6-1·§6-2) | 재무 숫자라 가장 신중 — 조사 슬라이스 먼저, 그다음 수정 슬라이스 |

미룸("이관 후 UI 정리" 절로): 테마 저장 위치, 서브차량 설정 페이지 배치, 차량관리
카드 라벨, 기사연동관리 폼/목록 UI 패턴.

### 미실시 목록(다음에 이어갈 것)

1. `linkedDriverManagementPage`/`linkedDriverClientsPage`/`billingSettingsPage`.
2. `receivableDetailPage`(미수금 상세) — §6-2 react-app 미수금 2건의 상세.
3. 모든 모달(운행 일지 세부 입력, 콜 상세, 차량/거래처 등록·수정 폼, 세금계산서 작성 등).
4. 온보딩 마법사, 회원가입 폼 내부.
5. §4·§6-1·§6-2 "연습운수/22나2222 관련 데이터 불일치"의 실제 원인 코드 추적.
