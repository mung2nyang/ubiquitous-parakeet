...
> 검증 로그(구 §1~§31) 전체를 그대로 옮긴 원본(동결, 재사용·수정 금지).
...
## 상세 원본 로그 (구 `docs/report.md` §1~§31, 2026-09-10 그대로 이관)

> 아래는 `docs/report.md`에서 슬라이스 진행 중 실시간으로 작성됐던
> 착수지시서·완료확인·리뷰 원문이다. 이관 과정에서 문구를 고치지
> 않았다 — 섹션 번호(§1, §3 등)도 당시 report.md 안에서의 번호
> 그대로다.
...
## 31. 홈 캘린더 헤더 슬라이스 완료 확인 (2026-09-10)

작업자 커밋(`e5a1e8b`) → 보리 push 확인(바로 반영) → CI green(verify
`34436710292`·deploy `34436710277` 둘 다 success). 정확히 지시한
2파일(`CalendarHeader.jsx`+`side-menu.css`)만 변경, diff 전체
대조로 설계 그대로 구현 확인(`settings-header` 래퍼+3칸 구조,
`.top-btn-group` 삭제, `.top-notification-btn`은 `z-index`·
`overflow: visible`만 남기고 고정 위치 3줄 제거). **감시관이 로컬
dev 서버(`npm run dev`)로 직접 브라우저 실측**(게스트 세션,
375×812 모바일 뷰):
1. 알림벨(왼쪽, 배지 "1" 정상 표시)+빈 가운데+햄버거(오른쪽) 한 줄
   구조 스크린샷으로 확인 — 설계대로.
2. 아래 배너 로고+"운행 일지"+연월 선택 줄은 완전히 그대로.
3. 스크롤 시 알림벨·햄버거 줄이 배너와 함께 위로 사라짐(스크린샷
   전/후 대조) — 보리가 확인해준 의도된 동작 그대로.
4. 알림벨·햄버거 버튼의 `onClick` 자체는 diff에서 무변경 확인(래퍼만
   바뀜) — 실제 클릭은 이번 세션 브라우저 도구 렌더링 이슈로 스크린샷
   대조까지만 진행, 온클릭 핸들러 코드 자체가 안 바뀐 것으로 충분히
   갈음.
