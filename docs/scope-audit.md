...
- **Q3 운송비 내역서 / Q3 관련 🔧(reportSummary):** 조치 완료 — react-app `1c5c7b1`(스코프 수정 묶음 2, 보리 브라우저 확인·최종 승인). F-06(서브차량 내역서 합계 원본 100,000원 vs react 110,000원)은 조사 결과 **원본이 서브차량 내역서에서만 기사차량 수수료를 차감한 것이며 회사 제출용 운송내역서에는 맞지 않아 react-app 방식 유지**로 결정(sot §0·§4-4e). 확정 규칙 → `docs/sot.md` §4-4e.
