...
> 최종 갱신: 2026-09-21 (**UI 이관 §1~§16 전부 완료**(보리 확정),
...
> 전수 조사 `[x]`(`docs/scope-audit.md`), 스코프 수정 묶음 1 `[x]`(`1413e8e`). 이관 감사 B
> 결과 등재(`docs/finance-parity.md`, 보리 결정 대기). 남은 조치·기능 작업은 `docs/roadmap.md`.
> 진행 중인 슬라이스 없음)
...
2026-09-19). 진행 중인 슬라이스 없음, 다음 슬라이스는 보리 지정 대기.

UI가 아닌 기능 작업이라 이관 범위에서 뺀 것(명시적 제외·AGENTS §2,
전부 `docs/roadmap.md` "이관 완료 후 진행사항"):
- 이관 완전성 감사 조치 6건(5-5·10-7·11-1·11-5·11-6·11-7) — `docs/roadmap.md`,
  근거는 `docs/migration-audit.md`. 이관 감사 B(계산 로직 수치 대조) 결과 `docs/finance-parity.md`의
  🟡 F-01~F-06·❓ F-07은 보리 결정 대기. 스코프 수정 묶음 2(운송비 내역서, F-06과 같은 코드 영역)는
  `docs/roadmap.md`.
...
- 이관 완전성 감사 B(계산 로직 수치 대조, 읽기 전용) — 결과 `docs/finance-parity.md`(31행: ✅ 17 · 🟡 6 · ⚪ 7 ·
  ❓ 1). 재현 명령·표 행 수 검증. 🟡 F-01~F-06·❓ F-07은 보리 결정 대기(roadmap).
- 고정노선 무스코프 호출부 전수 조사(읽기 전용) `[x]` — 결과 `docs/scope-audit.md`. 프로덕션 142줄
  ...
  수정 묶음 1·2. 확정 규칙 초안(§6)은 수정 완료 후 sot에 기록.
