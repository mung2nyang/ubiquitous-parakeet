...
> 최종 갱신: 2026-09-08 (**우선순위 전환(보리 지시): ④ 대신 "원본 vs
> react-app UI 전체 비교"(`docs/ui-comparison-report.md`)를 먼저 진행.**
> 홈 캘린더(§1)·사이드메뉴(§1-1) 항목은 보리가 직접 확인해 확정 완료 →
> 홈 캘린더 수정 5건 착수지시서 작성 완료(`docs/report.md`), **보리 착수
> 승인 대기 중.** ④(매출 탭 수치 불일치 조사)는 착수 전 상태 그대로 보류.)
...
## 지금 하는 일 (2026-09-08, 우선순위 전환)

**"원본 vs react-app UI 전체 비교"(보리 지시, 게스트·차주만) 진행 중.**
`docs/ui-comparison-report.md`에 화면별 발견사항 기록 — 화면 하나씩 보리가
직접 확인·확정하면 그 화면 수정 착수지시서 작성 → 승인 → 작업자 전달 순서로
진행(§1 홈 캘린더처럼).

- **§1 홈 캘린더 — 수정 5건 `[~]`.** `fe98184`가 깨뜨린 회귀 2건(정비비
  배지·다크 하이라이트)은 `f24bd8e`로 해결 확인(CI 초록·감시관 브라우저
  재확인 완료). **남은 문제 1개**: "마이페이지" 줄바꿈(§1-6) 최초 진단이
  틀렸던 것으로 밝혀짐(폰트 목록은 한글에 영향 없음) — 진짜 원인은
  `white-space: nowrap` 누락, 수정 지시 작성 완료(`docs/report.md`
  "수정 지시 3"), **작업자 수정 커밋 대기 중.**
- **§1-1 사이드메뉴 — 보리 확정 완료(4건).** 다음 슬라이스 후보(순서 반전·
  배너 이미지 복원·기사 관리 문구 중복 — 톱니바퀴/서브차량 설정 항목은
  범위가 훨씬 커서 별도 세션 필요).
- **§2~13(마이페이지~고객센터) — 아직 자동 비교 초안**, 보리 검토 전. 홈 화면
  슬라이스 끝나면 다음 화면부터 같은 방식으로 이어감.
...
완료** → **③ "월간 운송료 정산" 위젯 구조 통일 — `[x]` 전체 완료**(③-1~③-4
+ 분리설계 슬라이스 전부 확정) → **④ 매출 탭 수치 불일치·거래처/미수금
데이터 차이 원인 조사 — 다음 착수 대상, 아직 조사·착수지시서 작성 전.**
