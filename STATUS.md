...
> 최종 갱신: 2026-09-18 (§16 슬라이스 C(차량별 설정값 배선, 안전한
> 부분만) `[x]` push·CI 초록·보리 최종 승인까지 전부 완료. §16 A/B/C
> 전부 완료됐지만 "결제 및 수금 입력"·"고정 노선"까지 차량별로 바꾸는
> 건 별도 세션으로 남음(§16 자체는 `[~]`). **다음 할 일로 §16 슬라이스
> D(일일운행 입력 화면이 차량별 설정값 실제로 읽어서 반영) 지정**,
> 아직 착수지시서 없음. §15 슬라이스 E도 여전히 보류)
...
§16 슬라이스 C(차량별 설정값 배선, 안전한 부분만) `[x]` 완료 —
A/B/C 전부 끝났지만 §16 자체는 아직 `[~]`(아래 "다음 할 일" 참고).

§15 슬라이스 E(원본엔 있던 "연동기사·차주 각 계정에서 거래처
고정노선 설정" 기능, 완벽 이관 대상)는 **보리 판단으로 UI 대조 전체
완료 후 별도로 하루 잡고 재조사**하기로 보류(2026-09-18, 명시적 제외
항목·AGENTS §2). 브라우저 검증 때마다 고정노선 버튼이 안 보여
원인 추정→수정을 3차례 반복했지만 **매번 틀렸음** — 가장 최근엔
`LinkedDriverClientsPage.jsx`를 고쳤으나 보리가 실제 본 화면은
`OwnerScopedClientsView.jsx`였던 것으로 추정했는데, **이 추정 자체도
확신 없음**(3차 시도 중 하나일 뿐). `LinkedDriverClientsPage.jsx`
수정은 working copy에 미커밋 상태로 남아있으나 신뢰하지 말 것 —
재조사 시 처음부터 다시 볼 것. 관련 메모는 `docs/roadmap.md`(§16 준비)
에도 있음. `[x]` 아님.

## 다음 할 일

§15 슬라이스 E(거래처 고정노선)는 **UI 대조·이관 전체 완료 후** 하루
잡고 처음부터 재조사(보리 지시, 지금 착수 안 함 — 지금까지 3차 시도
다 빗나갔고 기존 추정 원인도 신뢰 안 함).

§16(기사차량 운행일지 설정 위치 재구성) A/B/C `[x]` 전부 완료 —
세부입력 5종+달력 표시방식은 차량별로 바뀜. **"결제 및 수금 입력"
토글 신설·"고정 노선"까지 차량별로 바꾸는 건 계산 엔진(매출/정산)을
같이 고쳐야 하는 별도 세션으로 미룸**(보리 지시, 2026-09-18) — 상세
설계 메모는 `docs/roadmap.md` §16 준비 메모 참고, 착수 시점은 보리
지정 대기.

**§16 슬라이스 D(다음 할 일로 지정, 2026-09-18, 아직 착수지시서 없음)
— 일일운행 입력 화면이 차량별 설정값을 실제로 읽어서 반영하게 배선.**
지금 §16 C는 "설정 화면"이 차량별로 값을 저장하는 것까지만 끝났고,
`DayLogPage.jsx`/`CallDetailForm.jsx`/`CallDetailCard.jsx`는 여전히
`logId`와 무관하게 항상 메인 `useOwnerSettings()` 값만 읽는다(확인
완료, 2026-09-18) — 즉 서브차량 설정화면에서 "계기판 입력"을 꺼도
실제 그 차량 일지 입력 화면엔 반영 안 됨. `MainPageRoute.jsx`가
`DayLogPage`에 넘기는 `settings`를 `logId` 기준으로
(`subCarSettings[logId]`) 스코프해서 내려주는 작업 필요. 착수 시
착수지시서 새로 작성.
...
- **react-app**: local·origin 동기, `b94eb30`까지(§16 슬라이스 C, CI
  초록·보리 최종 승인 완료).
