...
1. **§9(세금계산서) UI 대조 + `side-menu.css` H·J 블록 분리를 한
   슬라이스로 진행**(2026-09-16 결정, `docs/ui-comparison-report.md`
   상단 "`side-menu.css` 잔여 경계 지도" 참고) — G(리포트)가 §10보다
   먼저 CSS만 떨어져 나간 건 예외로 기록만 남기고, 앞으로 화면별
   블록(C/D/G/H/J/K/L)은 §순서 그대로 간다. B/E/F 같은 진짜 공용
   블록만 §순서와 무관하게 별도 시점에 정리.
...
- **react-app**: `main` = `4840148`(배너 이미지 경로 fix) — push됨,
  origin과 동일. CI·Deploy 초록·보리 실검증·AI 코드 리뷰·최종 `[x]` 완료.
