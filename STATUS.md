...
> `docs/sot.md` §0에 기록. **④정비/주유/기타 연결 구현·검증 완료(react-app
> `2f52ef4`), 3개 시나리오 실측 전부 통과 — ⚠️단 `LinkedDriverManagementPage.jsx`가
> 248→254줄로 사전 미승인 재초과(§11에서 안 다뤘던 부분, 감시관 실수), 보리
> 확인 대기 중.**
...
- **④정비/주유/기타 연결(마지막 4단계) — 구현·검증 완료, 보리 확인 대기**
  ...
  **→ 보리 결정(2026-09-09): 분할 대신 중복 제거로 줄이기 — 구현·검증 완료
  ...
  다크모드) 전부 정상.
  ⚠️ **단, 지시하지 않은 "정비/주유/기타" 칩 JSX 한 줄 압축이 diff에 같이
  포함됨**(동작 회귀는 없음, 실측 확인) — §5-1 범위 준수 항목만 경미하게
  위반. 되돌릴지 그대로 둘지 보리 확인 필요. 상세 `docs/report.md` §12-6~7.**
