...
> 최종 갱신: 2026-09-08 (**③-2 여전히 `[~]`.** 보리가 원본·react 화면
> 동시 대조로 지적한 지출 3행 아이콘·색상·구분선·순서 차이를 감시관이
> 원본 코드와 대조해 수정 지시 → 작업자 반영(`74e90ae`), CI 초록·감시관
> §5 통과. 색상 미세 차이 1건은 보리가 확인 후 수용. **남은 건 보리의
> 명시적 최종 `[x]` 승인뿐** — 감시관은 더 이상 임의로 `[x]`로 안 적음.)
...
**③-2(캘린더 카드 연결) — `[~]`(진행 중, `[x]` 아님).** `CalendarPage.jsx`가
...
2개 포함) 제거 — 보리 확인("원본대로") 반영. 부가세 계산도 원본대로(flat
10%, `vatExempt` 미고려)로 바뀜. 원본 수수료 들여쓰기 CSS(`#mainPage
.summary-client-commission-*`)를 `.main-page` 스코프로 포팅. — react-app
`e00a004`(5 files). 브라우저 검증 중 보리가 발견한 원본 불일치 2건 —
정산 카드 제목이 "총 N회 운행"이 아니라 "횟수 N회 · 세부 입력 M건"으로
돼 있던 것, 원본에 없던 하단 안내 문구("{이름}님 · 달력에 횟수 기록")·
로그아웃 버튼("처음으로 돌아가기", 원본 로그아웃은 개인정보 페이지
1곳뿐임을 감시관이 확인) — 을 같은 슬라이스 수정 커밋 `3045c45`로 반영
(2 files). 두 커밋 다 CI "verify" 초록(run `34176240684`/`34176962060`,
conclusion=success·headSha 일치·3게이트 green)·감시관 §5 7항목 통과(diff가
지시서·수정 지시문과 정확히 일치, 타입 꼼수 0, `.md` 변경 0, 127/117줄
(§6 이내), 신규 `CalendarMonthSummary.test.js`(136줄)+기존
`CalendarPage.test.js` 갱신 전부 실제 렌더 결과 대조). `showToast:
_showToast` 구조분해는 작업자 확인 결과 정당한 사유(부모 타입 호환,
동작 무변화)로 결론 — 별도 조치 불필요.

**⚠️ 정정(2026-09-08)**: 감시관이 보리의 "푸시햇어 브라우저도 봣고 ui만
아직이면 검증완"이라는 애매한 상태 보고를 명시 승인으로 잘못 해석해
`[x]`로 STATUS.md·`docs/report.md`에 기록했었다 — 보리가 "ui도 이게
끝이라고? 그럼 확정아니야"로 즉시 정정, `[~]`로 되돌림.

**실제 문제 확인됨(2026-09-08)**: 감시관이 원본(`ubiquitous-parakeet`
static-preview)·react-app(dev 서버) 두 화면을 동시에 띄우고 보리가 주석
단 캡처를 보내 대조 → 정비/주유/기타 지출 3행에 **아이콘(렌치/주유기/
목록)·색상(주유비 `--primary-color`, 기타 `--sunday-color`)·위쪽 점선
구분선이 원본엔 있는데 react엔 빠져 있었음**(이 3행 자체가 ③-2 전엔
카드에 없던 신규 행이라 처음부터 텍스트만 포팅됐던 것) + **행 순서**도
원본은 "기사차량 수수료 → 파렛트"인데 react는 반대였음. 감시관이 원본
`index.html:481-508`·`style.css:364-378`과 대조해 정확한 수정 지시문
작성 → 작업자 반영, `74e90ae`(3 files: `CalendarMonthSummary.jsx`+33·
`CalendarMonthSummary.test.js`+50 신규 케이스·`main-calendar.css`+17
`.inline-icon`/`.inline-icon.sm` 유틸 클래스). CI "verify" 초록(run
`34177766804`, conclusion=success·headSha 일치·3게이트 green)·감시관 §5
통과(diff가 수정 지시문과 정확히 일치, 타입 꼼수 0, 138줄). **보리:
"색 안 맞긴 한데 걍넘어가"** — 미세한 색상 차이(추정: 다크/라이트 테마별
`--primary-color`/`--sunday-color` 실제 값 차이) 1건은 보리가 확인 후
**의도적으로 수용, 추가 수정 안 함**[확인: 2026-09-08].

**남은 건 보리의 최종 `[x]` 확정 승인뿐** — 코드·CI·감시관 §5, 발견된
UI 문제 2건(제목 문구+안내/로그아웃, 아이콘·색상·순서) 전부 수정
완료. 색상 미세 차이는 위처럼 보리가 수용.
...
**이관 계획(보리 승인, 2026-09-07, 구 ③ 삭제로 재번호) — 순서대로**:
① 공지사항 이관 — **`[x]` 완료** → ②(①~④ 문자 문구 설정 전체) — **`[x]`
완료** → **③ "월간 운송료 정산" 위젯 구조 통일**(범위 확정: 캘린더+리포트
함께, ③-1/③-2/③-3 3슬라이스로 분할. **③-1(공용 계산 함수) `[x]` 완료**,
**③-2(캘린더 연결) 착수지시서 작성 대기** — `docs/report.md` 참고) →
④ 매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사(재무 숫자,
조사 슬라이스 먼저).
