...
> 뿐이었음. 보리 확인·승인("좋아"). **합쳐진 "거래처 연결" 착수지시서 작성
> 완료**(`docs/report.md` §7), 사용자 착수 승인 대기 중. 도메인 개념 정의는
> `docs/sot.md` §0에 기록.
...
- **계획 정정(2026-09-09)**: 2단계 착수 전 재확인하다 1차 조사가 틀렸음을
  ...
  **정비/주유/기타**는 원래 진단 그대로 유효(저장은 `vehicleNumber`로 잘
  ...
  데이터 확인 필요).
- **"거래처 연결" 착수지시서 작성 완료**(`docs/report.md` §7) — 1단계에서 만든
  `domain/driverManagementContext.js`를 그대로 재사용(새 헬퍼 불필요),
  `LinkedDriverClientsPage.jsx`를 두 모드 지원하도록 확장 + 라우트 1개 추가 +
  1단계 화면의 "거래처" 칩 연결선 1줄. 정확히 3파일(전부 수정, 신규 없음).
  사용자 착수 승인 대기 중.
