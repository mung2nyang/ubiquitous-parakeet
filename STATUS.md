...
**착수지시서 작성 완료 — 이관 계획 ① "공지사항 이관", 작업자 전달 대기
(2026-09-07).** 보리가 이관 순서를 승인("그 순서로 진행해") → 감시관이 `docs/report.md`에
착수지시서 작성 완료. 건드릴 파일 4개(신규 `NoticePage.jsx` + `lazyPages.js`·
`AppShellRoutes.jsx`·`MyPage.jsx` 각 1줄), 새 CSS·새 저장소 없음, §4 플레이북
비대상(정적 데이터). **다음: 작업자에게 `docs/report.md` 전달 → 코드 → `npm test`
→ commit(push 안 함) → 보리 push → CI → 감시관 §5 리뷰 → 보리 브라우저 검증 → `[x]`.**
