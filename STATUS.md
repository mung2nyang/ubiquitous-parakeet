...
> 최종 갱신: 2026-09-07 (이관 계획 ②-2 "콜상세 문자보내기 3종 복원" — 보리
> 브라우저 실검증 통과, **`[x]` 최종 확정**. 다음: 이관 계획 ②-3 "리포트
> 이미지 저장 버튼" 착수지시서 작성 중 — 공유 모달(카카오톡/문자)은 파일
> 수가 5~6개로 커져서 보리 질문 후 별도 ②-4로 분리.)
...
**이관 계획 ②-3 "리포트 이미지 저장 버튼" — 착수지시서 작성 완료, 작업자
전달 대기 (2026-09-07).** 이관 계획 ②-2(콜상세 3종 복원)는 `[x]` 최종
확정(아래 "완료" 절 참고). 착수 전 조사에서 원본 리포트 화면엔 PDF
다운로드(이미 이관됨) 외에 "한 장 이미지로 저장"(PNG, 단순)과 "거래처에
파일 보내기"(카카오톡 Web Share API+문자 SMS 공유 모달, 복잡)가 따로
있음을 확인 — 스코프를 보리에게 질문 → **"이미지 저장만 먼저(권장)"으로
확정**(2026-09-07), 공유 모달은 ②-4로 분리. 이번 슬라이스는 기존
`ReportPage.jsx`의 PDF 다운로드 버튼과 거의 같은 패턴(html2canvas 재사용)
으로 PNG 다운로드 버튼만 추가. 건드릴 파일 3개(`lib/report.js`에 파일명
함수 2개 추가, `ReportPage.jsx`에 버튼+핸들러 추가, `report.test.js`에
테스트 추가) — 새 CSS 없음(기존 `.theme-toggle-btn` 재사용). **`lib/report.js`
(198→약 213줄)·`ReportPage.jsx`(172→약 212줄) 둘 다 200줄을 넘길 것으로
예상돼 지시서에 §6 예외 사유주석을 명시**(파일명 함수들이 흩어지면 안
되는 응집도, PDF/이미지 두 내보내기 핸들러가 나란히 있어야 하는 응집도).
§4 플레이북 비대상(순수 표시·파일 생성, 저장·동기화 없음). **다음:
작업자에게 `docs/report.md` 전달 → 코드 → `npm test` → commit(push 안 함)
→ 보리 push → CI → 감시관 §5 리뷰 → 보리 브라우저 검증 → `[x]`.**
...
이미지 저장 버튼(착수지시서 작성 완료, 작업자 전달 대기)** → ②-4 리포트
카카오톡/문자 공유 모달(내역서 공유 문구 반영, 파일 5~6개 예상 — ②-3 완료
후 착수) → ③ 홈 캘린더 9/6 칩 렌더링 버그 → ④ "월간 운송료 정산" 위젯 구조
통일(원본 거래처별 vs react 횟수·단가별, 착수 전 보리 결정 필요) → ⑤ 매출
탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사(재무 숫자, 조사 슬라이스
먼저).
