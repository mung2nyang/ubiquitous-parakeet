...
>
> **사이드메뉴 UI 정리(§1-1) 1번(순서반전)·3번(배너 이미지) 구현·검증
> 완료**(react-app `c8647fc`, 보리 push) — CI green + 감시관 브라우저
> 실측(라이트·다크 순서 일치, 배너 토글 정확) §5 7항목 전부 통과, **보리
> 최종 승인 대기 중**(`docs/report.md` §13-8~9). 2번(톱니바퀴/
...
> 기사 관리"로 통일 §15 — **둘 다 착수지시서 작성 완료, 사용자 착수 승인
> 대기 중**(§13·§14·§15 순서로 진행 예정).
...
착수지시서 작성 완료(`docs/report.md` §13), 사용자 착수 승인 대기 중.
2번(톱니바퀴/세부입력 토글 메인·서브 분리)은 **보리 결정으로 이번 범위
...
해제: 이름 완전 선택, 전화번호는 입력 시에만 형식 검증)·**§15**(그 결과
사이드메뉴·"관리" 화면 제목을 "{기사이름 또는 차량번호뒷자리} 기사 관리"로
통일) 착수지시서 작성 완료, 사용자 착수 승인 대기 중.
