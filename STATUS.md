...
> 구현·CI green·감시관 §5 7항목 통과·브라우저 실측까지 완료, 보리 최종
> 승인 대기 `[~]`.** react-app `8c1ccc7`(작업자 커밋, 보리 push). 톱니바퀴
> (앱 설정) 아이콘이
> "찌그러져 보인다"는 기록을 감시관이 CSS/레이아웃으로 먼저 조사했으나
> 재현 못 함 → 보리가 실제 화면 스크린샷(빨간 밑줄)으로 다시 짚어줘서
> 재조사 → **원인은 CSS가 아니라 아이콘 SVG 좌표(`d` 속성) 자체의
> 오타**였음을 확정(마이그레이션 때 부호 `-` 1개 누락, 톱니 1개가
> 반대 방향으로 그려짐 — 원본·react-app 경로 문자열을 문자 단위로 대조 +
> 두 아이콘을 나란히 렌더링해 육안 확인까지 완료). 여기에 필수 원칙
> (2026-09-10, `docs/ui-comparison-report.md` 상단)대로 마이페이지 전용
> CSS 219줄(`side-menu.css`→신규 `mypage.css`)을 같은 슬라이스로 묶어
> `docs/report.md` §1(구 §42)에 착수지시서 작성, **보리 명시 승인
> "진행하라"(2026-09-10)** → **작업자 구현 완료 + 보리 push(`8c1ccc7`).**
> CI green(verify·deploy 둘 다 success, headSha 일치) + 감시관 §5 7항목
> 통과(정확히 지시한 3파일만 변경, 아이콘 `d` 값이 원본과 문자 단위로
> byte-identical, CSS 219줄 내용 변형 없이 순수 이동 확인) + 감시관
> 브라우저 실측(아이콘 6배 확대해 톱니 8개 대칭 확인, CSS 컴퓨티드
> 스타일 일치) — 상세 `docs/report.md` §1-1. **보리 최종 브라우저
> 확인·`[x]` 승인만 남음.**
...
> 컴포넌트화 우선"으로 방향 확정 → 감시관이 2,155줄 전수 조사 후
> Rule of Three 확실한 첫 패턴(차량/거래처/개인정보·앱설정 모달의
> `form-group`/`label`/`input-box` 완전 동일 중복 3곳) 발견 →
> `shared-controls.css`에 조합 셀렉터로 통합, `side-menu.css` 중복
> 9블록 삭제. 조사 중 발견한 `ReportDetailView.jsx`의 거래처 선택
> 모달(스코프 없어 미적용 상태였음)도 같이 통일하기로 보리 승인 —
> 새 스코프 클래스 `report-picker-modal` 1개 추가(작은 시각적 변화
> 1건, 사전 승인됨). CI green(`99c5c6e`) + 감시관 §5 7항목 통과 +
> 브라우저 컴퓨티드 스타일 실측(차량관리 모달·리포트 모달 일치
> 확인, 콜상세 폼은 스코프 밖이라 셀렉터 대조로 무영향 확인) —
> 도중 `shared-controls.css`가 187→215줄로 처음 200줄을 넘은 것을
> 착수지시서 단계에서 못 짚어 사후 발견, 보리 확인 후 1줄 주석
> 추가(`d58c05c`)로 마무리, CI green 재확인 — **`[x]` 확정.**
>
> 이어서 `pill-btn`/`dark-pill-btn`(용도·클릭동작·컨텍스트별 실제
> 모양 다 달라 **병합 보류로 결론**, 조사만 하고 손 안 댐) 조사 후,
> `.car-modal`/`.client-modal` shell(완전 동일 4선언, 5개 소비처) 통합
> `[x]`(`10a78cb`) + 죽은 클래스 `dark-pill-group` 제거 `[x]`(`803a1ac`)
> 완료 — CI green, 감시관 브라우저 실측(모달 컴퓨티드 스타일 일치·
> 콜상세 폼 3구역 레이아웃 무변화) 확인. 보리가 "shared-controls.css
> 분할 해야할거같다" 제기 → 감시관 검토 결과 224줄로 아직 급한 수준은
> 아니나(§6 ~250줄 예외 범위) 내용이 "범용 컨트롤"과 "모달 폼 패턴"
> 두 성격으로 갈라져 있어 분할 제안(`src/modal-form.css` 신규,
> `docs/report.md` §39) → 보리 승인 → **`[x]` 완료**(react-app
> `ee1ba1f`, 신규 `src/modal-form.css` 37줄 + `shared-controls.css`
> 187줄로 복귀 + `App.jsx` import 1줄, CI green + 감시관 브라우저
> 실측(모달 shell·form-group 컴퓨티드 스타일 분리 전/후 일치) 확인).
> 직전 커밋(`47f92af`, side-menu.css 여분 `}` 제거)은 감시관이 범위
> 밖으로 플래그했으나 **보리 본인이 파일 확인 중 직접 고쳐 커밋·
> push한 것으로 확인**(작업자 범위 규칙 비대상) — 문제없음.
>
> **오늘(2026-09-10) `side-menu.css` 다이어트 세션 마무리.** 전수조사
> 결과 남은 건 화면별 전용 스타일뿐, 더 뽑아낼 만한 3화면+ 진짜 중복은
> 없음(감시관 확인). **다음 순서 확정(보리 지시, 필수):
> `docs/ui-comparison-report.md` §2~14(보리가 직접 작성해둔 화면별 UI
> 대조 기록)를 착수하면서, 그 화면을 열어보는 김에 `side-menu.css`에서
> 그 화면 전용 CSS도 같이 떼어낸다 — UI 대조와 CSS 분리를 별도 세션으로
> 나누면 같은 화면을 두세 번 열어야 해서 비효율적이라 반드시 한
> 슬라이스로 묶는다.** (상세 원칙은 `docs/ui-comparison-report.md`
> 최상단에 기록.)
>
> 이전 최종 갱신: 2026-09-10 (**공용 헤더 컴포넌트(`PageHeader`) 통일
> 작업 — 전체 완료 `[x]`.** §1-2 원 계획 ①~⑤(화면별 복붙 방식)로
> 가다가 200줄 초과가 반복돼 보리가 중단시키고 "공용 컴포넌트 하나로
> 통일"로 방향 전환 → 22개 화면 전부 확인(3중 교차검증) → 신규
> `PageHeader.jsx` → ①~⑥ 슬라이스(매번 CI green + diff 대조 + 보리
> 승인)로 22개 전부 통일, 지난번 발견한 누락 3곳(고객센터·매출·
> 연동/미연동 기사관리 거래처)도 해소, 껍데기 파일 2개
> (`LinkedDriverManagementPage.jsx` 로컬 헬퍼·`revenue/RevenueNav.jsx`의
> `PageShell`) 흡수 → 마지막으로 홈 캘린더도 `[알림벨]+[공백]+[햄버거]`
> 한 줄 구조로 통일(감시관이 로컬 dev 서버로 직접 브라우저 실측:
> 레이아웃·스크롤 동작 확인). **상세 슬라이스별 이력은
> `docs/archive/pageheader-unification.md`로 이관**(동결, §1~§31 원본
> 착수지시서·검증 로그 전문 포함 — 2026-09-10 report.md 정리로 이제
> `docs/report.md`엔 안 남아있고 이 archive 파일에만 있음).
> ⚠️ 이 작업 중 감시관이 실수로 `git push`를 1회 직접 실행한 절차
> 위반 있었음(내용은 문제없었고 보리 양해받음) — 개인 메모리
> `watchdog-never-git-push.md`에 재발 기록.
>
...
>
> **사이드메뉴 UI 정리(§1-1) — 1·3·4단계 전부 `[x]` 완료(2026-09-09).**
> 1번(순서반전)·3번(배너 이미지) `[x]`(react-app `c8647fc`, §13). 미연동
> 등록 시 기사명·연락처 필수 해제 + 타이틀을 "{기사이름 또는 차량번호
> 뒷자리} 기사 관리"로 통일하는 작업은 두 슬라이스로 나눠 진행: §14(검증
> 해제, `ffc4e2d`)가 "기사 연동" 탭 선택 시에도 이름·연락처를 안 막아
> 버리는 착오가 있어 보리가 미승인 → §16(`bbaa4f4`)이 "기사 연동" 탭 +
> 로그인 세션일 때만 사전 검사로 막도록 정정, §15(`537cd48`)가 타이틀
> 통일 담당. **§15·§16 CI green + 감시관 코드/정적 검토 + 보리 직접
> 브라우저 실측까지 마치고 명시 승인 "승인"(2026-09-09)** — 둘 다
> `[x]` 완료. 상세 `docs/report.md` §14~§16. 2번(톱니바퀴/세부입력 토글)은
> 미연동 서브차량 일지 원본대조 작업 때 함께(보리 결정, 2026-09-09,
> 아래 "후속 nit" 참고). 4번("기사 기사 관리" 중복, 연동 기사 쪽)은
> 원본(`driver-link.js:29-33`)도 완전히 동일한 로직임을 확인 — 이관
> 버그 아님, 손 안 댐.
>
> **⚠️ `domain/cars.js` 200줄 초과 — 보리 확인 완료, 그대로 둠.** §14
> 착수 전부터 이미 219줄(기존 "§6 예외" 코멘트, 상한 미명시)이었고 §14+
> §16 거치며 237줄까지 늘어남 — 감시관이 §14 검토 때 놓쳤던 부분, §16
> 자체교차검증 중 뒤늦게 발견. 유일한 실질 중복(`getCarBusinessInfo`·
> `getVehicleSupplierIdentity`의 `ownerBiz` 리터럴, ~7줄)을 빼도 200줄
> 밑으로는 안 내려가 분리설계 없인 해소 불가 — 보리 "일단 알겠어"로
> 확인, 별도 조치 없이 보류(2026-09-09).
...
**사이드메뉴 UI 정리(§1-1) — 1·3번 + 후속 검증 규칙(§14~16) 전부 `[x]`
완료**(위 최종 갱신 절 참고, `docs/report.md` §13~§16). 2번(톱니바퀴/
세부입력 토글)은 "미연동 서브차량 일지 원본대조" 작업 때 함께 진행하기로
백로그 이동(아래 "후속 nit" 절). 4번("기사 기사 관리" 중복, 연동 기사
쪽)은 원본도 동일 로직이라 이관 버그 아님 — 손 안 댐.
**§1-2(공용 헤더/햄버거 버튼) — 전체 완료, 이력은
`docs/archive/pageheader-unification.md`로 이관**(위 최종 갱신 절
참고). 화면별 복붙 방식 ①~④-2까지 진행하다 200줄 초과 반복으로
중단 → 공용 컴포넌트(`PageHeader`)로 전환해 22개 화면 + 홈 캘린더까지
전체 통일 완료.

`docs/ui-comparison-report.md`의 옛 §2~13(감시관 자동 초안)은 폐기,
**보리가 직접 작성한 §2~14(마이페이지~고객센터) 비교 기록**은 아직
착수지시서 없음, 기록만 완료된 상태로 남아 있음(다음 착수 후보).
