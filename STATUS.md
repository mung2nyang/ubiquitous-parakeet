...
> 최종 갱신: 2026-09-08 (**③-3(리포트 요약 화면 연결) 작업자 커밋 `e71f82b`
> push 완료·CI "verify" 초록·감시관 §5 통과** — 상세는 `docs/report.md`.
> **지금은 보리 브라우저 실검증 → 최종 `[x]` 대기.** 검증 중 보리가 발견:
> 원본엔 있던 "중간 일자별 표"(`script.js` `createTableHTML`)가 react-app
> 리포트 요약 화면에 아예 없음 — ③-3 범위 밖(감시관 조사 누락으로 확인),
> ③-3 `[x]` 확정 후 **③-4**로 별도 착수지시서 작성 예정.)
...
**③-3(리포트 요약 화면 연결) 착수지시서 작성 완료 — `docs/report.md` 참고,
보리 착수 승인 대기.** 착수 전 조사로 원본 리포트 화면(`buildReportPage`,
`script.js:4789-5078`)이 캘린더 위젯(`updateSummary`)과 애초에 다른 화면임을
발견 — 캘린더엔 있고 리포트엔 없는 것(파렛트·서브차량수수료·지출3종·1회단가·
상단타이틀), 리포트엔 있고 캘린더엔 없는 것(월간 총 운행거리, 조건 없이 항상
표시)이 갈림. **보리 확인 → "원본 그대로"로 확정(2026-09-08)** — 리포트
화면에 파렛트·서브수수료·지출3종을 새로 추가하지 않음. `domain/day-record.js`의
`monthWorkFareSummary`는 프로덕션 소비처가 0이 되지만 `workData.test.js`가
직접 단위 테스트하는 대상이라 이번 슬라이스에서 삭제하지 않기로 지시서에 기록
(범위 확대 방지).
...
**③-2(캘린더 연결) `[x]` 완료**, **③-3(리포트 연결) 착수지시서 작성 완료,
보리 착수 승인 대기** — `docs/report.md` 참고) →
