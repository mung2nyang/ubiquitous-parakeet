...
> 최종 갱신: 2026-09-17 (§12 기사연동관리 슬라이스 1+2 + 이름/전화번호 버그 전체 승인)
...
**§12(기사연동관리) 슬라이스 1+2 + 이름/전화번호 소실 버그 전체 `[x]` 완료**
(2026-09-17, react-app `b88a5f0`+`2f050f0`) — 히어로 카드 제거·요약카드 통일,
기사 카드 재구성(세로글자 버그 수정·할당차량/기간 박스·버튼 재구성·문자발송
버튼 이관), 새로고침 시 기사 이름이 "기사"로 바뀌고 전화번호가 사라지던 버그
수정(`hydrateMerge.js`의 `mergeDriversFromRows`가 이미 차량 테이블에 저장된
driverName/driverPhone을 안 쓰던 이관 누락 — 원본 `driver-link.js` 방식대로
차량 쪽 값 최우선 순서로 수정). CI·Deploy 초록·`npm test` 170개 통과·
typecheck 0에러·보리 브라우저 실검증(새로고침 후 이름/전화번호 유지, 문자발송
버튼 정상) 완료.

## 다음 할 일

1. §12 끝났으니 다음 UI 대조 화면은 보리가 지정.
...
- **react-app**: `main` = `2f050f0`(§12 전체 완료, origin 동일, 보리
