...
**§3 거래처 수수료·파렛트 배지·수수료 입력 UI `[x]` 완료(2026-09-15).**
`1a3d6aa`→`d3585db`→`647344f`→`eedf5f5`→`2065755`. CI·보리 승인.

**§3 거래처 CSS 분리 `[x]` 완료(2026-09-15).** `9bb204f`,
`client-management.css`. CI 초록·보리 브라우저 실검증·최종 승인.

**미수금 UI 복원 `[x]` 완료(2026-09-15).** `12a9dbb`. CI·보리 승인.

**`side-menu.css` 분리 슬라이스 G(리포트) `[~]`** — `cf54899`,
`report/report.css`(233줄). side-menu 1421→1188. 브라우저 실검증·
push·`[x]` 대기. 경계 지도·구현기록 `docs/report.md` 이전 커밋
`7854094`.

**"원" 앞 공백 제거 `[~]`** — `241d4db`(`formatWon()`+테스트 21곳) +
`86d805e`(매출 수입/지출 합계 2곳, 착수지시서 승인 후 구현 —
지시서 없이 먼저 고쳤던 `383698d`는 `git revert`/`da6951c`로 되돌림).
브라우저 실검증(매출 화면 포함)·push·`[x]` 대기. 상세 `docs/report.md`.

**보리 지시(2026-09-14, 디자인 토큰 건에서 확인): "라이트 전용만
있는 건 다크모드도 적용해야 한다, 앱 통일을 위해서."** — 일반 원칙으로
기록. 토큰 파일 재검토 결과 이 원칙에 걸리는 나머지 사례는 없음
(`--fs-7` 1건은 이미 이 작업으로 해결됨). 앞으로
...
1. **`side-menu.css` 분리 슬라이스 G `[~]`**(위) — push·브라우저 검증·
   `[x]` 후 I/K/L 등 다음 덩어리(보리 확인). `docs/report.md` 이전
   커밋 `7854094`.
2. **"원" 앞 공백 제거 `[~]`**(위) — push·브라우저 검증·`[x]`.
   `docs/report.md`.
...
- §3 거래처 CSS 분리 `[x]` — react-app `9bb204f`
...
- §2(차량 관리) 전체 `[x]` — 기사연동 전환 버그 + 신규등록 connectMode
  ...
  전부 CI 초록·보리 최종 승인(2026-09-15).
- §2-1-C 차량관리 전용 CSS 분리 `[x]` — react-app `2e1fcac`, CI
  초록·보리 실검증·최종 승인(2026-09-14).
- §2-1-B 수정/삭제 아이콘 공용화(5곳) + 일일운행 세로쌓임 회귀 수정
  ...
  §5에 리뷰 항목 6(공용 CSS 클래스 영향범위) 신규 추가.
...
- **react-app**: `main` = `4e3aeea`(드롭다운 슬라이스 2 회귀 수정, 슬라이스
  1·2 둘 다 포함) — push됨, origin과 동일. §2·드롭다운 슬라이스 1·2 관련
  CI "verify" 전부 초록·보리 최종 `[x]` 승인 완료(2026-09-15).
- **ubiquitous-parakeet**: local `main`이 origin보다 1커밋 앞섬(`4aaf963`,
  아직 미푸시). 지금 세션의 문서 정리(archive 정책 변경 등)는 아직
  워킹카피 미커밋 상태. **AI는 push 안 함.**
