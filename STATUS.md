...
> 분리 작업 진행 중, 4단계 중 3단계 완료.** ①진입점 신설 `[x]`(`0cc99c3`)
> → ②거래처 연결 `[x]`(`17937e9`, "2단계 태깅 버그"는 조사해보니 존재하지
> 않는 문제였음이 드러나 "거래처 연결"과 합침, `docs/report.md` §6) →
> ③콜상세 폼(일일 세부 일지)에 스코프 거래처 연결 `[x]`(`70d14f5`, end-to-end
> 실측 완료). 보리 명시 승인 **"필요하다면 알겠어"**(2026-09-09,
> `domain/clients.js` 204줄=200줄+4 초과분 포함해 ②③ 함께 승인) — 상세
> `docs/report.md` §10-4. **남은 건 ④정비/주유/기타 연결뿐 — 착수지시서 작성
> 완료**(`docs/report.md` §11), 사용자 착수 승인 대기 중. 도메인 개념 정의는
> `docs/sot.md` §0에 기록.
...
- **④정비/주유/기타 연결(마지막 4단계) — 착수지시서 작성 완료, 사용자 착수
  승인 대기 중**(`docs/report.md` §11). 확인된 버그: `MaintFuelPage.jsx`가
  저장은 `vehicleNumber`로 잘 갈라지는 데이터를 필터 없이 전부 합쳐서 보여줌.
  설계: `domain/expenses.js`(이미 정확히 200줄)는 안 건드리고 신규
  `domain/expenseScope.js`에 `getExpensesForLog` 추가, `MaintFuelPage.jsx`에
  `logId` prop 연결(~205~210줄로 소폭 초과 사전 승인 포함).
...
**미연동 서브차량 데이터 분리 4단계**(보리 지시, 2026-09-09): ①관리 진입점
신설 `[x]` → ②③거래처 연결(콜상세 폼 포함) `[x]` → **④정비/주유/기타 연결
— 착수지시서 작성 완료(`docs/report.md` §11), 사용자 착수 승인 대기 중.**
완료되면 사이드메뉴 UI 정리(§1-1 확정 4건 중 순서반전·배너·중복텍스트 —
톱니바퀴 항목은 이 4단계로 해결됨)로 이어간다. `docs/ui-comparison-report.md`의
§2~13은 **보리 확인 결과 내용이 다 틀려 폐기됨**(2026-09-09), §1·§1-1만 유효.
