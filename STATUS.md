...
> `side-menu.css` 정리 트랙 전체 완료. §15 슬라이스 E(연동기사 거래처
> 고정노선 토글 — 기사 본인·차주 양쪽 화면) `[x]` push·CI 초록·보리
> 최종 승인 완료(`8c2bf61`). 남은 기능 작업(미연동 고정노선·§16 후속 등)은
...
- 미연동 기사 거래처 고정노선(`docs/sot.md` §4-4c — 그 운행일지 안에서
  1곳) — 계산부(차주 매출·달력 집계)까지 손대야 해서 별도 슬라이스.
