...
> 최종 갱신: 2026-09-07 (메인 캘린더 지출 칩·기사연동/운행일지 탭 버그·987be18
> 재오픈 수정 3건 전부 CI 초록+브라우저 검증까지 `[x]` 확정. 별도 세션에서 감시관
> §1 위반·허위 승인 기록 발견해 정정. 다음: 리포트 거래처별 세부 보고서 뷰 착수)
...
**없음 — 아래 세 건 전부 CI 초록 + 보리 브라우저 실검증까지 끝나 `완료` 절로
이동(2026-09-07).** 다음 작업은 "다음 할 일" 참고.

※ **저장소 상태 (2026-09-07 최종)**: react-app `origin/main`=`HEAD`=`f45d198`,
...
**[착수지시서 작성 완료, 2026-09-07] 리포트 "세부 내역서(거래처별)" 뷰 이관** —
보리 지시로 진행. 원본엔 월간 요약 외에 거래처별 세부 내역 뷰
(`isDetailReportView`)가 있는데 react-app `report.js`/`ReportPage.jsx`엔 이
개념 자체가 없음. 이관 우선순위 원칙상 새 방향이 아니라 원본에 있던 기능이므로
이관 본편 대상. 원본(`script.js:5080-5338`) 전체 조사 완료, 스코프 확인 2건
사용자 답변 완료(PDF 포함/서브차량 제외). **착수지시서 `docs/report.md` 완성 —
작업자 전달 대기.** 건드릴 파일: `lib/report.js`(신규 함수 3개, 기존
`getCallDetailCommissionAmount` 재사용)·신규 `ReportDetailView.jsx`·
`ReportPage.jsx`·`report.test.js`·`side-menu.css`(§3/§6 응집도 예외, 987be18과
같은 사유). 리포트 화면이 애초에 메인 차량 전용이라 원본의 서브차량 기사
수수료 줄은 이번 스코프 밖 — 아래 "후속 nit"에 알려진 이슈로 별도 기록.
