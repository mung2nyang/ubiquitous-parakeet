...
**§1~§3·미수금 UI 전부 `[x]` 완료 — §4~§14 UI 대조 + `side-menu.css`
분리 작업 진행 중.** §1 남은 토글 닫기 기능 1건만 `InlineSheet`
...
1. **"원" 앞 공백 제거 `[~]`**(위) — push·브라우저 검증·`[x]`.
   `docs/report.md`.
2. `side-menu.css` 분리 다음 덩어리(I/K/L 등) — 어느 것부터 할지
   보리 확인 후 착수. 경계 지도는 이전 커밋 `7854094`.
...
- §3 거래처 수수료·파렛트 배지·수수료 입력 UI `[x]` — react-app
...
- 전역 드롭다운 슬라이스 2(개별 select 5곳→`AppDropdown` 공용화)
  ...
  승인(2026-09-15).
- §1(일일운행) A~D·E-1·F-1~F-3·디자인 토큰 `[x]` — react-app
...
- §2(차량 관리) 전체 `[x]` — `f4de520`/`bc25009`/`dd54ca3`/`028af8e`.
...
- `ui-comparison-report.md` §2·§3·§1-A·일일운행 2부·홈 캘린더 ②③ 전부 `[x]`.
