...
**[착수, 2026-09-07] 리포트 "세부 보고서(거래처별)" 뷰 이관** — 보리 지시로 진행.
원본엔 월간 요약 외에 거래처별 세부 내역 뷰(`isDetailReportView`)가 있는데
react-app `report.js`/`ReportPage.jsx`엔 이 개념 자체가 없음(아래 "후속 nit"
참고). 이관 우선순위 원칙상 새 방향이 아니라 원본에 있던 기능이므로 이관
본편 대상. 감시관이 원본 코드부터 조사해 착수지시서 작성 예정.
...
### 후속 nit (급하지 않음)
- **리포트 "세부 보고서(거래처별)" 뷰 자체가 없음**[확인: 2026-09-05, 2026-09-05
  Step 10 3차 착수 조사로 발견] → **[착수, 2026-09-07]** 위 "다음 할 일" 참고,
  보리가 착수 지시함.
