...
**§9(세금계산서) 전체 `[x]` 완료**(2026-09-16, `e55c044`→`2ac7e73`→
`3e22169`) — UI 대조 5건·거래처 카드 세로 스택+칩 버튼 재구성·글씨·
간격·구분선·버튼폭 세부 조정까지 보리 브라우저 실검증으로 원본과
외관 일치 확인. AI 코드 리뷰(각 슬라이스 diff가 착수지시서와 정확히
일치, 신규 파일 없음, 타입꼼수 없음)·CI 초록·`npm test` 163개 전부
통과 확인. `side-menu.css` H·J 블록 분리도 §9 몫은 완료(`driver-code-row`만
§12용으로 의도적으로 남김). `.toggle-btn` 알약 라운드 공용 버그도
`[x]`(`eba8eb1`). **§1~§9 UI 전부 `[x]` — §10~§14 UI 대조 남음.** §4
남은 토글 닫기는 `InlineSheet` 리팩토링 뒤로 이월.

## 다음 할 일

- 기존 순서(§1~§14)상으로는 §10(운송비 내역서)이 다음이나, 확정된
  지시는 아직 없음 — 보리 지시 확인 후 착수.
...
- **react-app**: `main` = `3e22169`(§9 거래처 카드 세부 조정, origin
  동일). §9 전체(슬라이스 1·2 + 라운드 버그 + 세부 재작업 3연속)
  커밋·푸시·CI 초록·보리 실검증 완료.
- **ubiquitous-parakeet**: local이 origin보다 앞섬(`e585be0`까지 +
  이번 커밋 예정분). **AI는 push 안 함.** 정확한 HEAD는 매 세션 시작
  시 재확인(AGENTS §0-6).
