...
**활성 슬라이스 없음.** 연동 기사 거래처 스코프 불일치는 `[x]` 확정
완료(바로 아래 "완료" 절). 다음 슬라이스는 "다음 할 일" 목록에서 보리
지시로 선택.

## 다음 할 일

1. "부가세 해제" 레이블 16px 잔존[범위 밖 발견] — 다음 관련 슬라이스에서 처리.
2. §1-A 이후 — 보리가 직접 작성한 §2~§14 화면별 대조 기록을 순차로. 각 화면 =
