...
> 최종 갱신: 2026-09-24 (0-2 개인정보 대표자명·예금주 서버 저장·복원 `[x]` — react-app `43b61e5`(코드)·`f794946`(마이그레이션 0005 기록).
> 앞서 0-1 `[x]`(`914431c`)·③ 11-5·11-6 `[x]`(`41f9539`)·② 5-5 `[x]`(`550bca5`)·① 11-7 `[x]`(`fc4ec83`). 진행 중인 슬라이스 없음. 다음은 `docs/roadmap.md` "배포까지 순서" 3번(테스트 원본 의존 제거).)
...
1·2번(② 5-5, ③ 11-5·11-6) `[x]` 완료, 보리 지시 0-1(개인정보 로그인 입력 오류)·0-2(대표자·예금주 서버 저장)도 `[x]` 완료. 다음은 `docs/roadmap.md` "배포까지 순서" 3번(테스트 원본 의존 제거).
...
- 0-2 개인정보 대표자명·예금주 서버 저장·복원 `[x]` — react-app `43b61e5`(코드)·`f794946`(마이그레이션 0005 기록). 로그인 상태에서 입력한 두 값이
  `profiles`에 칸이 없어 서버에 안 올라가 새로고침·다른 기기 로그인 때 빈칸이 될 수 있었음. 보리가 Supabase에서 읽기 전용 쿼리로 진단(14칸, 두 칸 없음)한 뒤
  `profiles`에 `business_representative`·`account_holder`를 직접 추가(멱등 SQL, 확인 쿼리 2행, 기존 정책·데이터 무변경, 기사용 RPC는 이 칸을 안 돌려줌).
  코드는 `profileCloudCommit.js` upsert·`hydrateMerge.js` `mergeProfileRow`에 두 값 2줄씩 추가(개인정보 화면·`saveProfile`은 무수정).
  `npm test`(unit 683+화면 189)·`tsc` 0에러·strict 진단 새 파일 몫 0·수정 전 코드로 되돌리면 새 테스트 4개 FAIL·CI 초록·보리 브라우저 실검증·최종 승인 완료.
  DB 부분은 착수지시서 생략(보리 결정). **순서 교훈:** 코드보다 DB 칸이 먼저여야 함(없는 칸에 저장하면 프로필 저장 전체 실패).
- 0-1 개인정보 화면 로그인 상태 텍스트 입력 오류 `[x]` — react-app `914431c`. 로그인 상태에선 글자마다 서버 저장이 끝나야 화면 값이 바뀌어
  글자가 늦게 보이거나 사라졌음(2026-09-02 `fbe91d5`부터). `usePersonalInfoDraft`(신규)가 입력을 화면 임시값으로 즉시 보여 주고, 입력 멈춤 0.7초·
  칸 벗어남·화면 나감 시점에 기존 `saveProfile`로 묶어 저장(저장 중 추가 입력은 끝난 뒤 1회 더, 실패 시 토스트+저장 값으로 복귀). 새 큐·저장소 없음.
  `npm test`(unit 678+화면 189)·`tsc` 0에러·strict 진단 새 파일 몫 0·원래 코드로 되돌리면 핵심 새 테스트 3개 FAIL(원래 증상 재현)·CI 초록·
  보리 브라우저 실검증·최종 승인 완료.
...
- **비용 3종 테이블 RLS** — 정책은 라이브에 존재, 마이그레이션 파일엔
  없음(필요 시 `0006`으로 스냅샷화 — `0005`는 profiles 대표자명·예금주 칸에 쓰임).
- DB 마이그레이션 `0001`~`0005` 라이브 적용·검증 완료 ·
  `npm run typecheck` → 0 에러.

## 저장소 상태

- **react-app**: `f794946`까지(0-2와 마이그레이션 0005 기록, push 완료·코드 CI 초록·보리 최종 승인 완료).
