...
**`docs/ui-comparison-report.md` §1-A — 일일운행 콜상세 폼 CSS 4건.**
착수지시서 = `docs/report.md` §3. 상태 `[ ]` — **착수 전 보리 확인 대기**
(목표 상태 4건 그대로 진행해도 될지).

- 원칙(보리 지시 2026-09-10): "화면 UI 대조 + 그 화면 전용 CSS를 `side-menu.css`에서
  분리"를 **한 슬라이스로 묶는다**(같은 화면을 두 번 열지 않으려고).
- 4건: 헤더 날짜 누락 / 입력창 focus 파란테두리 전역 누락 / 계기판 "km" 위치 /
  레이블 글자크기(fs-3·fs-2 두 그룹). B그룹(누락 기능 3건)·시간입력 위젯은 별도.

## 다음 할 일

1. `ui-comparison-report.md` §1-A 이후 — 보리가 직접 작성한 §1-B(누락 기능) 및
   §2~§14 화면별 대조 기록을 순차로. 각 화면 = 대조 + 그 화면 전용 CSS 분리 한 슬라이스.
2. (§2~§14 중 이미 완료: §2 마이페이지, §3 매출 — 아래 "완료" 참고)
