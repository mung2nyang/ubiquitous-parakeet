...
> 최종 갱신: 2026-09-07 (서브 차량 지출 칩 이관 1단계 — CI 초록, 보리 브라우저 검증만 남음)
...
- **[2026-09-07] 작업자 구현 완료 — push 대기.** react-app `987be18`(16 files, 로컬
  ...
  build 3게이트 success). **남은 것: 보리 브라우저 실검증**(순서 `docs/report.md` §5
  끝부분: 메인/서브 칩 분리·`/app/expenses` 수정 시 태그 유지·새로고침 후 유지)
  → 통과하면 최종 `[x]`.
