...
> 착수지시서 작성 완료, 사용자 착수 승인 대기.** 감시관이 코드 확인해
> `side-menu.css`의 `.top-menu-btn.icon-btn`·`.top-notification-btn.icon-btn`
> 오버라이드(원형·투명)가 뒤로가기 버튼 기본 `.icon-btn`(사각·카드배경)과
> 다른 원인임을 특정, 중복 규칙(§7 기존 버그, 새 레이어 아님)까지 확인.
> 11개 화면에 햄버거 버튼 자체가 없는 것도 확인(②~⑤ 슬라이스, 총 5슬라이스
> 계획). 상세 `docs/report.md`. 이전 작업(미연동 서브차량+사이드메뉴, §1~§16
...
**§1-2(공용 헤더 스타일 통일) — 감시관이 코드 확인 후 5슬라이스로 분할,
① CSS 통일 착수지시서 작성 완료(2026-09-10, `docs/report.md`), 사용자
착수 승인 대기 중.** 보리 확정: 슬라이스 분할(①CSS 1파일 → ②~⑤ 화면
3~4개씩 묶어 햄버거 버튼 추가) + 기사연동관리는 연동·미연동 두 라우트
모두 적용. 상세·다음 슬라이스(②~⑤) 로드맵은 `docs/report.md` §2 참고.
