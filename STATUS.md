...
> 최종 갱신: 2026-09-07 (이관 계획 ① "공지사항" — 작업자가 코드 작성 후
> "CI 3게이트 통과" 보고했으나 감시관 확인 결과 **아직 커밋 전**(실제 GitHub Actions
> CI 미실행). 감시관이 diff·원본 대조·로컬 3게이트 재실행으로 §5 체크리스트는
> 통과 확인, 그러나 절차상 커밋→push→실제 CI가 남아 `[~]` 유지 — 상세는
> `docs/report.md` §6.)
...
**이관 계획 ① "공지사항 이관" — 코드 작성 완료, 커밋 전 `[~]` (2026-09-07).**
작업자가 지시서 4개 파일(신규 `NoticePage.jsx` + `lazyPages.js`·`AppShellRoutes.jsx`·
`MyPage.jsx` 각 1~수줄) 코드 작성 후 "CI 3게이트 통과·브라우저 확인 부탁"으로 보고.
**감시관 확인 결과 아직 `git commit` 전**(작업 트리에 unstaged 3개+untracked
1개뿐, 실제 GitHub Actions "verify" 실행 이력 없음 — 작업자가 말한 "CI"는 로컬
`npm test`/`typecheck`/`build` 직접 실행 결과였음). 감시관이 diff를 지시서·원본
`index.html:713-723`과 직접 대조 + §5 7항목 체크(범위·증설·타입·200줄·테스트·문서·
요구사항 전부 통과, `NoticePage.jsx` 65줄) + 로컬 typecheck/test(593+138)/build
3게이트 재실행으로 회귀 없음 확인. 사소한 편차: 빈 줄 2곳(로직 무관, 커밋 전 정리
권장). **다음: 작업자가 커밋(한국어 메시지, push 금지) → 보리 push → 실제 CI
"verify" 초록 확인 → 그 다음 보리 브라우저 검증(마이페이지→공지사항→3건 확인→
펼침/접힘→뒤로가기) → `[x]`.** 상세 `docs/report.md` §6.

원본↔React UI 전수 대조 1차 조사 전체 내용(재무 숫자 불일치 등 나머지 발견 항목)은
`docs/archive/audit.md` "원본↔React UI 전수 대조 — 1차 조사" 절로 옮김(동결, 참고용).
이관 계획 ②~⑤는 ①이 `[x]` 확정된 뒤 순서대로.

### 이번 조사에서 발견한 것 (요약, 상세는 `docs/report.md`)
...
- **개별 승인 기록이 있는 건 차주 뱃지 삭제 1건뿐** — 테마 저장 위치(로컬↔클라우드)·
  ...
  "승인된 변경"으로 잘못 분류했었음, `docs/report.md` 참고).
- **일치 확인된 화면**: 일일운행·매출(차주 탭)·정비/주유/기타·세금계산서·개인정보·
  고객센터.
- **이번엔 안 본 것**: 기사 연동 상세·Billing·미수금 상세·모든 모달·온보딩 폼 내부
  (`docs/report.md` "다음 세션에서 이어갈 것" 참고).
- **이관 계획(제안, 보리 승인 대기)**: `docs/report.md` "이관 계획" 절 참고 — ①
  공지사항 이관 → ② 문자 문구 설정 이관 → ③ 홈 캘린더 9/6 칩 렌더링 버그 → ④
  "월간 운송료 정산" 위젯 구조 통일(원본 거래처별 vs react 횟수·단가별, 보리 결정
  먼저 필요) → ⑤ 매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사(재무 숫자라
  가장 신중하게, 조사 슬라이스 먼저).

※ **저장소 상태 (2026-09-07)**: react-app `origin/main`=`HEAD`=`eb734e7`, 클린,
미push 커밋 없음. ubiquitous-parakeet은 `origin/main`=`da4ab76`, 클린, 미push
커밋 없음(세션 시작 시 `git fetch`로 직접 확인). 이번 조사 기록(`STATUS.md`+
`docs/report.md`)은 감시관이 커밋만 하고 push는 보리가.
...
**이관 계획(보리 승인, 2026-09-07) — ①부터 순서대로**:
① 공지사항 이관(착수지시서 작성 완료, `docs/report.md`, 작업자 전달 대기)
→ ② 문자 문구 설정 이관 → ③ 홈 캘린더 9/6 칩 렌더링 버그 → ④ "월간 운송료 정산"
위젯 구조 통일(원본 거래처별 vs react 횟수·단가별, 착수 전 보리 결정 필요) →
⑤ 매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사(재무 숫자, 조사 슬라이스
먼저).
