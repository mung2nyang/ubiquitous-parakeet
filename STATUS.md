...
> 둘 다 `[x]` 완전 종료, 보리 push 완료·CI·Deploy 초록 확인)
...
다음 슬라이스는 보리 지정 대기. 아래 두 슬라이스 전부 완료(아래
"완료" 절로 이동 정리함).

## 다음 할 일

1. 다음 UI 대조 화면·작업은 보리 지정 대기.
...
- **react-app**: local·origin 둘 다 `c906ca4`로 일치(보리 push 완료,
  CI·Deploy 초록 확인).
