...
> 분리 작업 진행 중.** 1단계(관리 진입점 신설) `[x]` 완료(react-app `0cc99c3`,
> 보리 명시 승인 "승인"). **2단계 착수 전 재확인하다 1차 조사 오류 발견 →
> "2단계(거래처 태깅 버그)"·"3단계(거래처 연결)"를 "거래처 연결" 하나로 합침**
> (`docs/report.md` §6) — "콜상세에서 새 거래처가 태그 없이 자동 생성된다"고
> 했던 게 틀렸음, 그런 자동 생성 경로 자체가 없고 그냥 전용 화면이 없을
> 뿐이었음. 보리 확인·승인("좋아"). **"거래처 연결" — react-app `17937e9`,
> CI green + 감시관 §5 7항목 통과 + 스코프 격리 양방향 실측 확인(전용 화면에
> 등록한 거래처가 일반 목록엔 안 보이고, 그 반대도 안 섞임).** 보리가 이어서
> **"콜상세 폼(일일 세부 일지)에도 스코프 거래처를 연결해야 한다"고 지시**
> (`docs/report.md` §9) — 지금은 콜상세 입력 화면이 어느 일지인지 모르고
> 항상 일반 거래처만 자동완성에 보여줘서, 방금 만든 전용 거래처가 그 일지에서
> 안 보이는 문제. 착수지시서 작성 완료, 사용자 착수 승인 대기 중. 도메인
> 개념 정의는 `docs/sot.md` §0에 기록.
...
- **계획 정정(2026-09-09)**: 2단계 착수 전 재확인하다 1차 조사가 틀렸음을
  ...
  **정비/주유/기타**는 원래 진단 그대로 유효(저장은 `vehicleNumber`로 잘
  ...
  데이터 확인 필요).
- **"거래처 연결" `[~]`** — react-app `17937e9`(정확히 지시한 3파일, 전부 수정
  ...
  **스코프 격리 양방향 확인**(미연동 전용 화면에서 등록한 거래처가 일반
  ...
  보리 최종 승인 아직 전(§9와 함께 나중에 받기로 함, `docs/report.md` 8-4).
- **콜상세 폼(일일 세부 일지)에 스코프 거래처 연결 — 착수지시서 작성 완료,
  사용자 착수 승인 대기 중**(`docs/report.md` §9). 보리가 이어서 지적:
  "거래처 화면 만든 것과 별개로, 그 서브차량 운행일지에서 콜상세 입력할
  때도 그 거래처가 자동완성에 보여야 한다." 원인: `CallDetailForm.jsx`가
  `logId`를 모른 채 항상 일반(미연동) 거래처만 자동완성·즐겨찾기에 보여줌.
  `domain/clients.js`에 `getClientsForLog(clients, logId)` 신규 + `DayLogPage.jsx`가
  이미 갖고 있는 `logId`를 `CallDetailForm`에 전달 — 정확히 3파일(전부 수정,
  신규 없음).
- **남은 백로그**: "정산·계산서 설정"→"운송내역서"(이름만, 공제후지급액 고정은
  별도 결정), "상세 설정"→"정비/주유/기타"(이름만, 나중 단계에서 연결).
