...
> 최종 갱신: 2026-09-08 (**이관 계획 ③ 전체(③-1~③-4 + 분리설계 슬라이스)
> `[x]` 확정 — "월간 운송료 정산" 위젯 구조 통일 완료.** 보리 명시
> 승인("승인"). 상세는 아래 "완료" 절. **다음은 ④(매출 탭 수치 불일치·
> 거래처/미수금 데이터 차이 원인 조사) 착수 — 아직 조사·착수지시서 작성 전.**)
...
**다음 할 일 = ④ 매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사.**
아직 착수 전 — `docs/archive/audit.md` "원본↔React UI 전수 대조 — 1차 조사"
절의 §4·§6-1·§6-2(순이익 부호 반대, "연습운수" 거래처, 미수금 0건 vs 2건 —
전부 서브차량(22나2222) 관련 데이터 처리 차이로 추정)부터 다시 확인하고
조사 슬라이스 착수지시서 작성 필요.
