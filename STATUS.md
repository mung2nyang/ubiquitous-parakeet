...
**미수금 UI 복원 `[x]` 완료(2026-09-15).** `12a9dbb`, `receivables.css` +
목록/카드/상세를 원본 `.receivable-*` 구조로 이식(버튼 글자 깨짐
등 원인 5가지 수정). CI 초록·보리 브라우저 실검증·최종 승인.
...
1. **`side-menu.css` 분리작업** — 보리 지시(2026-09-15). 전체 13개
   경계 지도 + 슬라이스1 제안(리포트/운송비내역서 233줄, 4파일만
   소비·블라스트반경 0) `docs/report.md`, 착수 승인 대기.
