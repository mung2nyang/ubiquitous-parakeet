...
> 재배치됐다. **다음은 원본↔React UI 전수 대조의 다음 화면 — 이관 계획 ④
> "매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사"부터 재개, 다음
> 세션에서 착수지시서 작성.** 상세 `docs/report.md` §20~§25.)
...
**다음 순서**: 이관 계획 ④ "매출 탭 수치 불일치·거래처/미수금 데이터 차이
원인 조사"부터 재개(그동안 CSS 책임 분리 우선으로 보류돼 있던 항목) — 다음
세션에서 재조사·착수지시서 작성. 그 뒤 원본↔React UI 전수 대조에서 아직
안 본 나머지 화면(기사 연동 상세·Billing·미수금 상세·모든 모달·온보딩 폼
내부 등, `docs/archive/audit.md` "미실시 목록" 참고)으로 이어간다.
