...
**§1 남은 순서(F-2 → E → D):**
1. **F-2. 다크모드 드롭다운(`:-webkit-autofill`) 입력창 밝아짐** `[~]` —
   `shared-controls.css` 1개 파일, 착수지시서 완료·승인 대기.
2. **E. 정비/주유/기타 패널** `[ ]` — **착수 전 보리가 나머지 항목부터
...
3. **D. 시간입력 위젯(`app-temporal` 대체)** `[ ]` — 새 공용 컴포넌트
...
- **react-app**: `main` = `8ced672`(F-1 포함), 클린, origin과 동기화.
- **ubiquitous-parakeet**: `main` = `f54d9e5`. STATUS·report.md 수정 중
  (F-1 `[x]` 확정 반영, 아직 미커밋 — 모아뒀다 커밋). **AI는 push 안 함.**
