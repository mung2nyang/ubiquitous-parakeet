...
> 고객센터·매출·연동/미연동 기사관리 거래처 4화면 누락 발견 후
> 200줄 초과 처리를 또 상의하던 중, 보리가 "자꾸 쪼개지고 줄수가
> 늘어나서 안되겠다"며 **화면마다 헤더 JSX를 복붙하는 지금 방식 자체를
> 중단**시키고 **"모든 화면이 공유하는 공용 헤더 컴포넌트 하나"**를
> 만들라는 새 방향 지시. **감시관은 착수지시서를 쓰지 말고 보리가
> "착수지시서 만들어"라고 할 때까지 대기**(명시 지시, 그 전까지 §1-2
> ⑤ 이후 신규 착수지시서 작성 금지). 참고로 조사해보니 `settings-header`
> 패턴을 쓰는 파일이 22개(`grep -rl 'settings-header'`) — 공용화하면
> 지금 겪는 200줄 창발적 문제가 구조적으로 해소될 가능성 높음(감시관
> 관찰, 지시 아님). 이전 작업(미연동 서브차량+사이드메뉴, §1~§16 전체
> `[x]`)은 `docs/archive/sub-vehicle-management-and-side-menu.md`로 이관.)
>
