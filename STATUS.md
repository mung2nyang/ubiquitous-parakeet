...
> ③ "홈 캘린더 9/6 칩 렌더링 버그"는 보리 확인 결과 실제로 재현 안 됨 —
> 1차 조사 당시 오판으로 확인돼 계획에서 삭제.** 다음 항목이 ③으로
> 승격 — 상세는 아래 "지금 하는 일".)
...
**다음 항목이 ③으로 승격**: "월간 운송료 정산" 위젯 구조 통일(원본은
거래처별, react-app은 횟수·단가별로 다르게 집계) — 착수 전 **보리 결정
필요**: 어느 구조로 통일할지(원본 방식으로 되돌릴지, react 방식을 유지하고
지출 3종 요약만 추가할지, 다른 방식으로 재설계할지).
