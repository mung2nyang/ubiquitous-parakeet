...
**연동 기사 본인 로그의 거래처 스코프 불일치 — 아직 착수지시서 없음, 조사
착수 예정.** 게스트 데이터 유실 버그는 `[x]` 최종 확정 완료(바로 아래
"완료" 절). 기사 본인 계정 하이드레이트(`hydrateEmployedDriver.js`)가
차주 거래처 전체를 스코프 필터 없이 내려받는데, 이 기사의 운행일지는
항상 `logId='main'`으로 취급돼 `getClientsForLog`가 "스코프 없는(차주
일반) 거래처"만 보여준다 — 기사 전용 거래처(`OwnerScopedClientsView.jsx`
기준)와 다른 집합일 수 있음("거래처 +추가" 슬라이스 조사 중 발견).

## 다음 할 일

1. **연동 기사 본인 로그의 거래처 스코프 불일치 조사·수정** — 착수지시서
   작성부터 시작.
2. "부가세 해제" 레이블 16px 잔존[범위 밖 발견] — 다음 관련 슬라이스에서 처리.
3. §1-A 이후 — 보리가 직접 작성한 §2~§14 화면별 대조 기록을 순차로. 각 화면 =
...
- **react-app**: `main` = `0b9358d`(게스트 데이터 유실 버그 수정, CI green). 클린.
