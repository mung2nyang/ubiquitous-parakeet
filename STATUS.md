...
> (`docs/report.md` §9) — react-app `70d14f5`, CI green + 감시관 §5 7항목
> 사실상 통과 + **end-to-end 실측**(메인/서브 일지 콜상세 자동완성이 정확히
> 갈리고, 저장한 콜상세가 "2222 관리" 화면 정산·거래처 세금계산서에 문자열
> 완전 일치로 집계됨). **⚠️ `domain/clients.js`가 185→204줄로, 사전 승인 없이
> 200줄을 넘김(+4, 늘어난 내용 전부 필요한 로직·주석) — 보리 확인 필요.**
> §7~8·§9 둘 다 아직 `[x]` 미확정, 이 200줄 건 포함해 함께 승인받을 예정.
> 도메인 개념 정의는 `docs/sot.md` §0에 기록.
...
- **콜상세 폼(일일 세부 일지)에 스코프 거래처 연결 — 구현·검증 완료**(react-app
  ...
  세금계산서에 정확히 집계). **⚠️ `domain/clients.js`가 185→204줄로 사전
  승인 없이 200줄 초과(+4) — 보리 확인 필요.** §7~8("거래처 연결")·§9(이 슬라이스)
  둘 다 아직 `[x]` 미확정, 이 200줄 건 포함해 함께 승인받을 예정.
