...
`1a3d6aa`(배지 2종) → `d3585db`(색상 통일) → `647344f`(배지 자리 고정)
→ `eedf5f5`(수수료 입력 UI 원본 일치+패널 배경) → `2065755`(모달 토글버튼
라운드 12px 통일, 이식 누락분). 전부 CI 초록·push 완료·보리 브라우저
실검증·최종 승인. 상세 경위(착수지시서 누락 2건 포함) `git log -p --
docs/report.md` 참고.
...
1. **§3 CSS 분리(보리 지시, 2026-09-15)** — `side-menu.css`에서
   거래처 전용 규칙 분리(기존 초과분). 착수지시서 `docs/report.md`에
   작성 후 승인 대기(§3 화면 전체 UI 대조는 미완이지만, 보리 지시로
   대조 완료를 기다리지 않고 CSS 분리부터 진행).
