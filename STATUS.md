...
> 최종 갱신: 2026-09-07 (이관 계획 ②-4 "리포트 카카오톡/문자 공유 모달" —
> 보리 push+CI초록+브라우저검증 전부 완료 보고, 감시관이 커밋·CI·diff
> 9개 파일 전부 직접 재확인 후 **`[x]` 최종 확정 — 이관 계획 ② 전체 완료**.
> 다음: 이관 계획 ③ "홈 캘린더 9/6 칩 렌더링 버그" 착수 전 보리 확인 필요
> — 상세는 아래 "지금 하는 일".)
...
**이관 계획 ③ "홈 캘린더 9/6 칩 렌더링 버그" — 착수 전 재확인 필요
(2026-09-07).** 이관 계획 ②-4(카카오톡/문자 공유 모달)는 `[x]` 최종
확정(아래 "완료" 절 참고, 보리가 push+CI초록+브라우저검증을 한 메시지로
한 번에 보고 → 감시관이 커밋·CI·diff 9개 파일 전부 사후 재확인) — **이걸로
이관 계획 ② "문자 문구 설정"이 ①~④ 전부 완료.**

**③ 착수 전 감시관이 코드를 직접 읽어본 결과**: `domain/calendarBadges.js`
(`dayExpenseBadgeLabel`)·`components/calendar/CalendarGrid.jsx`·`CalendarPage.jsx`
3파일 전부 논리적으로 맞아 보인다(메인 캘린더는 `expenseVehicle=undefined`로
"차량 태그 없는" 지출만 합산, 서브 차량 지출 칩 관련 선행 슬라이스(`987be18`·
`f45d198`)가 이미 이 로직을 정리해둠). 그런데 원래 버그 보고("2026-09-06
정비 20,000원이 정비/주유/기타 화면엔 보이는데 홈 캘린더 칩엔 안 보인다")는
그 두 슬라이스보다 **나중에** 진행된 "원본↔React UI 전수 대조 1차 조사"에서
나온 것 — 즉 이미 그 수정들이 반영된 상태에서도 여전히 재현됐던 버그다.
코드만 읽어선 원인이 안 보여서, **실제 데이터로 재현해봐야 원인을 좁힐 수
있다** — 감시관은 로그인 테스트 계정 정보가 없어 직접 재현 불가.

**보리에게 확인 요청**: 최근(공지사항 이관 이후) react-app을 다시 열어
2026년 9월 홈 캘린더를 봤을 때 6일에 정비 지출 칩("2만")이 지금도 안
보이는지 한 번만 확인 부탁드립니다 — 그 사이 다른 슬라이스가 우연히 고쳤을
수도 있고, 아니면 진짜 원인을 더 들여다봐야 할 수도 있습니다. 안 보인다면
그 날짜의 정비 지출 항목 등록 방식(메인 차량 일지에서 직접 입력했는지,
서브 차량에서 입력 후 보이는 건지 등)도 같이 알려주시면 원인 좁히기가
빨라집니다.
...
완료** → **③ 홈 캘린더 9/6 칩 렌더링 버그(착수 전 보리 재확인 대기 — 위
"지금 하는 일" 참고)** → ④ "월간 운송료 정산" 위젯 구조 통일(원본 거래처별
vs react 횟수·단가별, 착수 전 보리 결정 필요) → ⑤ 매출 탭 수치 불일치·
거래처/미수금 데이터 차이 원인 조사(재무 숫자, 조사 슬라이스 먼저).

**추가로 발견한 관련 문구 버그(③과 별개, 미배정)**: 홈 화면 하단
"{이름}님 · 달력에 횟수 기록" 문구가 `inputMode` 설정과 무관하게 항상
고정 텍스트로 뜸(`CalendarPage.jsx` 맨 아래) — 원본엔 이 문구 자체가 없고,
`inputMode==='fare'`인 계정에서도 "횟수 기록"이라고 잘못 표시됨(1차 조사
§1에서 "확인 필요"로 남겨둔 항목). ③ 처리 중 발견했지만 별개 버그라 이번엔
안 건드림 — 필요해지면 별도 슬라이스.
