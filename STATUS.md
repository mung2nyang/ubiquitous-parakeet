...
**디자인 토큰(CSS 변수) 1곳 통합** `[~]` — react-app `ef82306`,
**push 완료·CI 초록 확인(AI 재검증 2026-09-14)**: `npm test`(791개)/
`typecheck`/`lint`/`build` 전부 통과, 코드 diff 재검토로 `--shadow-sm`
승자 값·다크 전용 변수(`--icon-color` 등) 보존 확인. 부작용 1건 발견
(문제 아님): `--fs-7`이 예전엔 라이트 전용이라 다크에서 미정의였는데
이번에 공용 `:root`로 옮겨져 다크에도 값이 생김 — 다른 `--fs-*`
전부 이미 공용이라 일관성 쪽으로 맞춰진 것. **보리 스크린샷 대조·
`[x]` 대기.** (§1 UI 대조는 이 작업 `[x]`까지 일시중단)

D·E-1·F-3까지는 전부 확정 완료(react-app `e65b2c0`/`b14fcd8`/`c395ab3`/
`93a4f2c`/`9e6f93d`/`83de8e9`). **E의 나머지 항목**(P0: `useExpenseForm.js`
`setTimeout(420)` vs `day-log.css` `0.4s`)은 토큰 통합 `[x]` 뒤 재개.
...
- **ubiquitous-parakeet**: `main` = `fc25629`. STATUS·report.md 수정 중
  (토큰 통합 push·CI 재확인 반영, 아직 미커밋). **AI는 push 안 함.**
