...
> 최종 갱신: 2026-09-09 (**`main-calendar.css` 책임 분리 1~7차 `[x]`, 8차
> 콜상세 카드(call-detail-card.css) — 작업자 구현·push 완료(react-app
> `a731ca1`), CI green + 감시관 §5 7항목 통과 + 라이트/다크 컴퓨티드 스타일
> 13항목 전수 대조 완전 일치. 보리 최종 승인 대기 중 `[~]`.** 재조사 중
> 발견한 `.action-icon-btn` 합쳐진 선택자(콜상세 카드+정비/주유/기타 공유)는
> 6차 선례대로 `day-log-shell.css`로 이동, 지시대로 정확히 4파일만 변경.
> **남은 건 콜상세 폼(call-detail-form.css) 1개 슬라이스 — 그게 끝나면 §3
> 설계표 전체 완료.** 상세 `docs/report.md` §20~§21.)
...
- **7차 콜 목록·일일 합계(call-detail-list) CSS 분리 `[x]`.**
  ...
  2026-09-08 — 세션 종료.
- **8차 콜상세 카드(call-detail-card) CSS 분리 — 구현·검증 완료, 승인 대기
...
7개 슬라이스 **전부 `[x]` 완료** → **⑧ 콜상세 카드(call-detail-card.css) —
구현·CI green·감시관 §5+브라우저 실측 완전 일치까지 끝남, 보리 최종 승인
대기 중(`docs/report.md` §21)** → ⑨ 콜상세 폼(call-detail-form.css) 재조사·
착수지시 → ⑩ `main-calendar.css` 해체 완료 → ⑪ 원본↔React UI 비교의 다음
화면 수정 재개. 기존 이관 계획 ④(매출 탭 수치 불일치·거래처/미수금 데이터
차이 원인 조사)는 그 뒤까지 보류한다.
