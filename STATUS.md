...
> `side-menu.css` 정리 트랙 전체 완료. 고정노선: §15 슬라이스 E(연동기사
> 거래처 토글, `8c2bf61`)·미연동 기사 거래처 고정노선 + 차주 집계 서브차량
> 스코프(`c8939c2`) 모두 `[x]` push·CI 초록·보리 최종 승인 완료. 남은 기능
> 작업(무스코프 전수 조사·§16 후속 등)은 `docs/roadmap.md`. 진행 중인
> 슬라이스 없음)
...
- 고정노선 무스코프 호출부 전수 조사(읽기 전용, 산출물은 분류표 →
  확정 규칙은 `docs/sot.md`에 기록) — `docs/roadmap.md`.
