...
> 최종 갱신: 2026-09-07 (서브 차량 지출 칩 이관 1단계 — 작업자 구현 완료, 감시관 사전점검 통과, push 대기)
...
- **[2026-09-07] 작업자 구현 완료 — push 대기.** react-app `987be18`(16 files, 로컬
  ...
  필요). 상세 `docs/report.md` §4~5. **남은 것: 보리가 push → CI 초록 확인 →
  브라우저 실검증(서브 로그 지출 등록 시 서브 캘린더에만 칩, 메인엔 안 뜨는지) →
  최종 `[x]`.**
