...
> 최종 갱신: 2026-09-07 (이관 계획 ②-3 "리포트 이미지 저장 버튼" — 보리
> push+CI 초록+브라우저 검증 전부 완료 보고, 감시관이 커밋·CI·diff 직접
> 재확인 후 **`[x]` 최종 확정**. 다음: 이관 계획 ②-4 "리포트 카카오톡/문자
> 공유 모달" 착수지시서 작성 중.)
...
**이관 계획 ②-4 "리포트 카카오톡/문자 공유 모달" — 착수지시서 작성 완료,
작업자 전달 대기 (2026-09-07).** 이관 계획 ②-3(이미지 저장 버튼)은 `[x]`
최종 확정(아래 "완료" 절 참고, 보리가 push+CI초록+브라우저검증을 한 번에
보고 → 감시관이 커밋·CI·diff 전부 사후 재확인). 이번 슬라이스는 미뤄뒀던
"거래처에 파일 보내기" 모달(카카오톡 Web Share API + 문자 SMS, 각각
PDF/이미지 선택) 이관 — 예상대로 파일이 많다: **신규 2개**(`lib/reportExport.js`
— html2canvas/html2pdf로 File 객체 생성, `components/ReportShareModal.jsx`
— 모달 UI+공유 로직)+**신규 테스트 1개**(`ReportShareModal.test.js`, `mock.module`
로 `reportExport.js`를 목킹해 실제 canvas/PDF 생성 없이 5케이스: 렌더 구조·
SMS 연락처 없음 alert·카카오 미지원 안내·카카오 지원 시 공유 호출(②-1 문구
반영 증명)·SMS URL 순수 함수 UA 분기)+**기존 4개 수정**(`lib/messageTemplates.js`
+`fillReportShareMessagePattern`, `lib/report.js`+`getReportShareCompanyName`/
`getDetailReportClientContact`(둘 다 순수 함수, `report.test.js`에 테스트 추가),
`ReportPage.jsx`+공유 버튼·모달 마운트, `side-menu.css`+`.report-share-*`
12규칙(ID 접두사 없이 이식)). **`lib/report.js`(218→약 234줄)·`ReportPage.jsx`
(225→약 240줄) 둘 다 §6 250줄 한도에 근접 — 사유주석 갱신 + "다음에 또
건드리면 분리설계 필요"를 지시서에 명시.** SMS 발송 시 `window.location.href`
직접 대입·`URL.createObjectURL` 실제 호출 경로는 jsdom이 못 다뤄(선례:
②-2도 같은 이유로 URL 생성만 순수 함수로 분리해 테스트) 브라우저 실검증으로만
커버(기존 `handleDownloadPdf`/`handleDownloadImage`도 컴포넌트 테스트 없는
선례와 동일 방침). §4 플레이북 비대상. **다음: 작업자에게 `docs/report.md`
전달 → 코드 → `npm test` → commit(push 안 함) → 보리 push → CI → 감시관 §5
리뷰 → 보리 브라우저 검증 → `[x]`.** 이걸로 이관 계획 ② 전체 완료 예정.
...
① 공지사항 이관 — **`[x]` 완료** → ②-1 문자 문구 설정(설정화면만) — **`[x]`
완료** → ②-2 콜상세 문자보내기 3종 복원 — **`[x]` 완료** → ②-3 리포트
이미지 저장 버튼 — **`[x]` 완료** → **②-4 리포트 카카오톡/문자 공유 모달
(착수지시서 작성 완료, 작업자 전달 대기, 이걸로 이관 계획 ② 전체 완료)** →
③ 홈 캘린더 9/6 칩 렌더링 버그 → ④ "월간 운송료 정산" 위젯 구조 통일(원본
거래처별 vs react 횟수·단가별, 착수 전 보리 결정 필요) → ⑤ 매출 탭 수치
불일치·거래처/미수금 데이터 차이 원인 조사(재무 숫자, 조사 슬라이스 먼저).
