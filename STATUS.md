...
**§11 개인정보 `[~]`**(2026-09-17) — MY PROFILE 삭제·섹션 간격·계정
부제·사용중 문구 삭제·탈퇴 밑줄 링크·`PersonalInfoPage.css` 분리.
코드·`npm test` 170개 통과, **브라우저 실검증 대기.** §10은 `[x]`.

## 다음 할 일

1. **§11 브라우저 실검증 → 통과 시 커밋**(푸시는 보리)
2. 다음 UI 대조 화면은 보리가 지정
