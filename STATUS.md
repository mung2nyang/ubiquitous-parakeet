...
> 차량 등록 후 목록 유지 — 보리 브라우저 실검증까지 끝나 `[x]` 확정, `완료` 절로
> 이동. 다음 건 착수 예정: "원본↔React UI 전수 대조" — 아래 "다음 할 일" 참고)
...
**없음 — 관리 화면 공통 네비게이션 정합성(뒤로가기 원위치 복귀 + 차량 등록 후
목록 유지)까지 보리 브라우저 실검증 끝나 `완료` 절로 이동(2026-09-07).** 다음
작업은 "다음 할 일" 참고.
...
**원본↔React UI 전수 대조** (보리 지시, 2026-09-07) — **감시관이 직접 조사**:
바닐라(`ubiquitous-parakeet`)와 react-app 화면을 동일한 데이터·화면 크기·테마·
권한 조건에서 감시관이 직접 비교하고, 화면별 차이와 근거(파일·줄번호·스크린샷 등)를
보고서(`docs/report.md`)에 작성한다. **이 단계에서는 코드 수정·완료(`[x]`) 판정을
하지 않는다** — 차이 발견·기록까지만.
- **역할 분담**: 감시관=전체 화면 차이 조사(전수). 사용자(보리)=로그인 지원·
  "이 차이는 의도된 것"인지 결정·수정 후 실기기 최종 승인만 담당(전수 비교 자체를
  사용자에게 넘기지 않음).
- 착수 전 확인 필요: 비교 대상 화면 범위(전체 vs 우선순위 화면)·비교 기준(원본
  로컬 실행 방법)·테스트 계정 등은 이 세션이 아직 안 물어봤음 — 다음 세션 시작 시
  질문 1회로 확정 후 진행.
