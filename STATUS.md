...
> 최종 갱신: 2026-09-09 (**`main-calendar.css` 책임 분리 — 1~10차 전부 `[x]`,
> §3 설계표 전체 완료.** 마지막 10차(해체)는 react-app `18a1693`, CI green +
> 감시관 §5 7항목 통과 + 라이트/다크 컴퓨티드 스타일 13항목 전수 대조 완전
> 일치(홈·매출·일지) 확인 후 보리 명시 승인 **"승인"**(2026-09-09)으로 닫음.
> 원본 1,282줄 `main-calendar.css`가 스텁 1줄만 남기고 전부 전용 파일로
> 재배치됐다.
>
> **⚠️ 이관 계획 ④ "매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사"는
> 삭제됨(보리 명시 지시, 2026-09-09) — 아래 §번호에서도 제거.** 예전에 이미
> "그거 계획한애가 헛소리한거니까 지워"라고 죽인 항목인데, 그 지시가 문서에
> 엉뚱하게(구 "홈 캘린더 9/6 칩 렌더링 버그" 항목으로) 잘못 기록되면서 원래
> 죽였어야 할 이 항목은 안 죽고 백로그에 남아있다가 오늘 감시관이 다시 "다음
> 할 일"로 잘못 꺼낸 사고 발생 — **감시관이 사용자 승인 없이 이 항목을 "고쳐야
> 할 일"로 되살려 놓은 것.** 보리 확인: "매출 탭은 수치 문제 없음 — 실제
> 원인은 기사일지·메인일지 쪽 버그이고, 아직 감시관에게 설명 안 한 상태."
> 즉 이 문제 자체가 없는 게 아니라, 원인 소재가 매출 화면이 아니라 다른
> 곳이며 보리가 나중에 직접 설명할 예정. **다음 세션은 이 항목을 조사·착수
> 대상으로 다시 꺼내지 않는다** — 보리가 별도로 설명할 때까지 대기.
...
대신 원본 vs react-app UI 전체 비교(`docs/ui-comparison-report.md`)로
이어간다 — 단, 그 문서의 §1(홈 캘린더)·§1-1(사이드메뉴)만 보리가 실제로
검토·확정한 것이고 **§2~13(마이페이지~고객센터)은 "자동 비교 초안"이며
보리 확인 결과 내용이 다 틀렸다(2026-09-09)** — 폐기, 참고 금지. 확정된
사이드메뉴(§1-1) 4건 중 수정이 아직 안 된 게 있는지부터 다음 세션에서
확인·정리한다.
