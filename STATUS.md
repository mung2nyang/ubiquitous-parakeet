...
**다음 슬라이스 미착수.** 버그1(2중 스크롤)·버그2(카드 이탈) 수정지시서 작성
전 — 아래 "다음 할 일" 참고.

- 원칙(보리 지시 2026-09-10): "화면 UI 대조 + 그 화면 전용 CSS를 `side-menu.css`에서
  분리"를 **한 슬라이스로 묶는다**(같은 화면을 두 번 열지 않으려고).

## 다음 할 일

1. **일일운행 2중 스크롤 버그**[보리 지적 2026-09-10] — `main-app-container`
   패딩(128px)과 `work-log-page`의 `100dvh`가 안 맞아 body에도 스크롤 발생.
   원인 확정, 착수지시서 미작성. 상세 `docs/report.md` §2-1.
2. **"N일 일지 세부 입력"이 카드 밖으로 나감**[보리 지적 2026-09-10, 원인
   확정] — 원본은 제목·목록·펼치는 폼이 한 카드(`modal-section`) 안인데,
   react는 `CallDetailList`가 카드를 닫아버리고 `InlineSheet`가 형제로
   따로 렌더링됨. JSX 구조 문제, 수정지시서 작성 필요. 상세 `docs/report.md` §2-2.
3. B그룹(즐겨찾기 칩·거래처 +추가 버튼·산재보험료 필드, `ui-comparison-report.md`
   §1-B) — 별도 슬라이스.
...
- **`ui-comparison-report.md` §1-A 일일운행(콜상세 폼 CSS 4건)** — 헤더 날짜·
  ...
  `[x]` 2026-09-10. 상세 `docs/report.md` §1-1.
...
- **react-app**: `main` = `c6c831b`(콜상세 폼 CSS 4건 — §3 슬라이스 직전 커밋).
