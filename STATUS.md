...
**B그룹 ② 즐겨찾기 칩(상차지/하차지) 착수지시서 작성 완료, 보리 확인 대기.**
① 산재보험료는 `[x]` 완료(react-app `1de2537`, 보리 브라우저 실검증
2026-09-11 통과). ②는 전체 이력 구독(`useOwnerWorkData` 재사용)이 필요해
파일 8개짜리 슬라이스 — 상세 `docs/report.md`.

B그룹 3개 슬라이스(보리 확인 2026-09-11): ①산재보험료 `[x]` → ②즐겨찾기 칩
→ ③거래처 +추가 버튼.

## 다음 할 일

1. B그룹 ② 즐겨찾기 칩(상차지/하차지, `settings.pinnedLocations` 신규 필드 필요).
2. B그룹 ③ 거래처 +추가 버튼(직원기사 등록 제한 권한로직 이관 필요).
...
- **B그룹 ① 산재보험료 필드** — react-app `1de2537`. 보리 `[x]` 2026-09-11.
  상세 `docs/report.md`(B그룹 ② 착수 전 archive 예정).
