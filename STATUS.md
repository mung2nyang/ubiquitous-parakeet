...
>
> **새 작업 착수: 미연동 서브차량(연동 안 된 기사차량 — 차주가 대신 일지를
> 쓰는 차) 관리 진입점 신설.** 보리가 실제 버그(기사일지가 메인일지로 섞이는
> 문제)를 설명 → 감시관이 코드 대조로 확인: 거래처는 콜상세 입력 시
> `scopedToVehicleNumber` 태그가 아예 안 붙어 메인 거래처 목록에 완전히 섞이고,
> 정비/주유/기타는 저장은 차량별로 잘 되는데 화면(`MaintFuelPage`)이 필터 없이
> 전부 합쳐서 보여줌. 4단계 해결 계획(①진입점 신설 ②거래처 태깅 수정 ③거래처
> 연결 ④정비/주유/기타 연결) 중 **①번 — react-app `0cc99c3`, CI green + 감시관
> §5 7항목 통과 + 게스트 실데이터로 정산 계산까지 실측 확인(콜상세 300,000원
> 등록 → 정산 요약·거래처 세금계산서 정확히 반영). `linked-driver.css` 0줄
> 변경 확인(보리가 걱정하던 부분).** 보리 최종 승인 대기 중 `[~]`. 도메인 개념
> 정의는 `docs/sot.md` §0에 새로 기록.
...
보리가 실제 버그(기사일지↔메인일지 데이터 섞임)를 직접 설명 — 4단계 계획의
①(관리 진입점 신설) **구현·검증 완료, 보리 최종 승인 대기 중.**

- **문제 확인(코드 대조, 사용자 승인 없이 코드 수정 안 함)**: 미연동 서브차량(연동
  기사 계정 없이 차주가 대신 일지를 쓰는 기사차량)의 **거래처**는 콜상세 입력 시
  `scopedToVehicleNumber` 태그가 안 붙어 메인 거래처 목록에 완전히 섞임(예전
  "연습운수" 미스터리의 유력 원인). **정비/주유/기타**는 저장은 `vehicleNumber`로
  잘 갈라지는데 `MaintFuelPage.jsx`가 필터 없이 전부 합쳐서 보여줌. **운송비
  내역서**는 보리가 의도적으로 서브차량 지원을 빼놓은 것(버그 아님).
- **도메인 개념**: `docs/sot.md` §0에 "미연동 서브차량도 실제 기사가 있고 정산이
  ...
  정정받은 경위 포함.
- **4단계 계획**: ①진입점 신설(연동 기사 관리 화면·메뉴 재사용) → ②거래처 태깅
  버그 수정 → ③관리 화면에 거래처 연결 → ④관리 화면에 정비/주유/기타 연결.
  **①번 `[~]`** — react-app `0cc99c3`(정확히 지시한 5파일: 신규
  `domain/driverManagementContext.js` 53줄 + 수정 4). 기존
  `LinkedDriverManagementPage.jsx`의 정산 계산 함수들이 `link` 없이도 안전하게
  동작한다는 §1 확인이 실데이터 실측(콜상세 300,000원 → 정산요약·세금계산서
  정확히 반영)으로도 증명됨. `linked-driver.css`는 **0줄 변경**(기존
  `.linked-driver-chip` 클래스 재사용) — 보리가 걱정하던 side-menu.css류 CSS
  거대화 없음. CI green + 감시관 §5 7항목 통과. 보리 최종 승인 대기 중.
- **이번 결정으로 별도 백로그 추가**: "정산·계산서 설정" 칩을 "운송내역서"로
  이름만 바꿔두고(공제후지급액 고정은 별도 결정 시), "상세 설정" 칩을 "정비/
  주유/기타"로 이름만 바꿔둠(④에서 실제 연결). 사이드메뉴 톱니바퀴 버튼 복원은
  **취소** — 대신 미연동 서브차량 일지 화면 자체에 나중에(그 화면 UI를 원본과
  맞추는 별도 작업 때) 넣기로 함.
...
**현재 착수 대상 — 미연동 서브차량 데이터 분리 4단계**(보리 지시, 2026-09-09):
①관리 진입점 신설(**착수지시서 작성 완료, 사용자 승인 대기**, `docs/report.md`)
→ ②거래처 태깅 버그 수정 → ③거래처 연결 → ④정비/주유/기타 연결. 완료되면
사이드메뉴 UI 정리(§1-1 확정 4건 중 순서반전·배너·중복텍스트 — 톱니바퀴 항목은
이 4단계로 해결됨)로 이어간다. `docs/ui-comparison-report.md`의 §2~13은
**보리 확인 결과 내용이 다 틀려 폐기됨**(2026-09-09), §1·§1-1만 유효.
