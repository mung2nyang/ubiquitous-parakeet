...
> 최종 갱신: 2026-09-18 (§16 슬라이스 B(설정화면 라우트 연결) `[x]`
> push·CI 초록·보리 최종 승인까지 전부 완료. §15 슬라이스 E는 여전히
> 보류. §16 슬라이스 C(차량별 설정값 실제 배선)는 다음 착수 대상)
...
§16(기사차량 운행일지 설정 위치 재구성)은 A(톱니바퀴 버튼+프로필
카드 이름/전화번호 표시, `[x]`)/B(설정화면 라우트 연결, `[x]`)/
C(차량별 설정값 실제 배선) 3슬라이스로 진행 중(보리 지시, 2026-09-18).
**다음 착수 대상은 슬라이스 C** — 착수지시서는 보리 지정 시 작성.
설계 방향은 `docs/roadmap.md` §16 준비 메모 참고(설정을 jsonb
`settings.subCarSettings = { [차량번호]: {...} }`로 차량별 재설계,
기존 공용 `subFixedOn` 등 값은 버리고 메인과 동일한 `defaults`로
시작).
...
- **react-app**: local·origin 동기, `a8115c4`까지(§16 슬라이스 B, CI
  초록·보리 최종 승인 완료).
