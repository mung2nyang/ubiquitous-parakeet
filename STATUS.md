...
**게스트 데이터 유실 버그 — 진짜 원인 확정(배포앱에서 직접 재현·검증),
보리 확인 완료(2026-09-11), 작업자 전달 대기.** (이전에 StrictMode를
원인으로 지목했던 건 오진 — 보리가 배포앱에서도 재현된다고 알려줘서
재조사, 정정함.) **진짜 원인:** B그룹① 산재보험료 슬라이스 이후 콜상세가
항상 `insuranceFee: ''`(안 채우면 빈 값)을 저장하는데, 저장 스키마 검증기
(`isValidCurrencyAmount`)가 빈 문자열을 무효로 처리 → 그 콜상세 하나가
아니라 **그 owner의 Store 초기화 전체가 무산**돼 앱이 "빈 계정"처럼
보임(디스크 `localStorage`엔 데이터가 멀쩡히 남아있음 — 화면에만 안
뜨는 것). 배포앱에서 실제로 필드 하나 지우고 새로고침해 정상화되는 것까지
확인해 100% 확정. 수정안(`lib/callDetailSchema.js` 1개 파일, 3줄)은
`docs/report.md` 참고 — 작업자에게 그대로 전달하면 됨.

## 다음 할 일

1. **게스트 데이터 유실 버그** [x] 보리 확인 2026-09-11 — 착수지시서
   `docs/report.md` 참고, 작업자 전달.
