...
- **§1 홈 캘린더 — 수정 5건 `[~]`, 보리 브라우저 검증 중 회귀 2건 발견.**
  react-app `fe98184` 커밋·push·CI 초록까지 갔지만, 보리가 다크모드
  실검증에서 fe98184가 새로 깨뜨린 문제 2건(정비비 배지 다크에서 사라짐,
  날짜 칸 클릭 하이라이트 여전히 흰색)을 찾음 — 감시관이 원인 확정,
  수정 지시 작성 완료(`docs/report.md`), **작업자 수정 커밋 대기 중.**
