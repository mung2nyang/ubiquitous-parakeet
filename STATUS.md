...
> 최종 갱신: 2026-09-07 (이관 계획 ②(①~④) 전체 `[x]` 완료, 구 이관 계획 ③
> "홈 캘린더 9/6 칩 렌더링 버그"는 삭제(오판 확인). **이관 계획 ③ "월간
> 운송료 정산" 위젯 구조 통일 — 조사 중 스코프 질문 던졌는데 보리가
> "중지, 여기까지만 기록하고 내일 아침 다시" 지시 → 코드 변경 없이 세션
> 종료.** 다음 세션 재개 시 아래 "지금 하는 일"의 질문부터 다시.)
...
**다음 항목이 ③으로 승격 — "월간 운송료 정산" 위젯 구조 통일, 조사 중 일시
중지 (2026-09-07 저녁, 보리 지시로 내일 아침 재개 예정).** 보리가 위치를
"캘린더카드 하단의 정산카드" 맞다고 확인 → "원본대로 수정해" 지시 → 감시관이
코드 조사 착수 → **원본 `updateSummary`(script.js:3529-3634) 로직이 생각보다
큼**을 발견(거래처별 매출·수수료 스냅샷·파렛트·기사차량 수수료·실운행거리
전부 포함) → 게다가 **홈 캘린더 카드와 리포트(운송비 내역서) 요약 카드가
같은 계산 함수(`monthWorkFareSummary`/`buildMonthReport`)를 쓰고 있어서
캘린더만 고치면 두 화면 표시 방식이 서로 안 맞게 됨**을 발견 → 다행히
거래처별 분류·수수료 계산 자체는 `lib/report.js`의 `buildDetailReport`
(세부 내역서 화면용)에 이미 비슷한 게 있어 재사용 가능할 것으로 보임 →
**보리에게 스코프 질문(캘린더만 먼저 vs 캘린더+리포트 요약 화면 같이)**
던짐 → **보리: "중지, 여기까지만 기록하고 내일 아침 다시 작업"** → 감시관
**코드 변경 없이 세션 종료**(Store/파일/DB 전부 변경 없음, 문서 기록만).

**다음 세션 재개 시 할 일**: 아래 질문에 보리 답변 먼저 받고 착수.
1. **캘린더만 먼저(권장)** — 홈 화면 카드만 원본처럼 거래처별+지출 3종
   구조로 고침. 리포트 요약 화면은 나중에 별도 슬라이스.
2. **캘린더 + 리포트 요약 화면 함께** — 계산 함수를 하나로 통일해서 양쪽
   다 원본과 같은 구조로. 파일 수·검토 시간 더 필요.
`docs/report.md`에 조사 내용 상세 기록해둠.
...
완료** → **③ "월간 운송료 정산" 위젯 구조 통일**(원본 거래처별 vs react
횟수·단가별, 착수 전 보리 결정 필요) → ④ 매출 탭 수치 불일치·거래처/미수금
데이터 차이 원인 조사(재무 숫자, 조사 슬라이스 먼저).
