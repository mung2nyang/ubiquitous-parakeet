...
> 최종 갱신: 2026-09-08 (**③-4(리포트 일자별 표 이관) 기능 완료(작업자
> `cc5583d`, push·CI 초록·§5 기능리뷰 통과)했으나 `ReportDetailView.jsx`
> 333줄·`lib/report.js` 290줄로 §6 200줄 상한을 사전 승인 없이 초과해
> `[x]` 보류.** 보리 결정: "분리설계 후속 슬라이스로 진행" — 감시관이 분리
> 설계안을 `docs/report.md`에 작성, **지금은 보리 착수 승인 대기.** 이
> 슬라이스 끝나면 ③-4도 함께 `[x]`.)
...
**③-3(리포트 요약 화면 연결) `[x]` 확정(2026-09-08, 보리 명시 승인
"승인") — 상세는 아래 "완료" 절.**

**③-4(리포트 일자별 표 이관) 기능은 완료(작업자 `cc5583d`, push·CI 초록,
감시관 §5 기능 리뷰 통과)했으나 `[x]` 보류 중.** 이유: `ReportDetailView.jsx`
333줄·`lib/report.js` 290줄로 AGENTS §6 "~250줄까지" 상한을 사전 승인 없이
초과(작업자가 절차 없이 주석만 "~330줄 허용"으로 바꿔 진행) — §6은 이 경우
분리설계안을 먼저 보고·승인받게 정함. **보리 결정: "분리설계 후속 슬라이스로
진행"** → 감시관이 분리설계안 작성 완료(`docs/report.md`), **보리 착수 승인
대기.** 승인·완료되면 이 슬라이스와 ③-4를 함께 `[x]` 처리.
...
함께. **③-1(공용 계산 함수) `[x]` 완료**, **③-2(캘린더 연결) `[x]` 완료**,
**③-3(리포트 연결) `[x]` 완료**, **③-4(중간 일자별 표) 기능 완료·`[x]`
보류**(§6 200줄 분리설계 후속 슬라이스 착수 승인 대기 — `docs/report.md`
참고)) →
