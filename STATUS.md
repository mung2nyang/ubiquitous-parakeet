...
> 기사 관리"로 통일 §15. **§15는 구현·커밋·push 완료**(react-app `537cd48`,
> 보리 push) — CI green(`CI`·`Deploy GitHub Pages` 둘 다 success), 감시관
> 정적 대조(범위 4개 파일 정확히 일치·타입 꼼수 0·200줄 이내·테스트
> 진실성)까지 통과. **다만 게스트 계정에 등록된 서브차량이 없어 15-5가
> 요구하는 4개 시나리오 브라우저 실측은 아직 못 함** — `537cd48`은 `[~]`
> 유지, 게스트 테스트로 실측 + 보리 최종 승인 남음. 상세
> `docs/report.md` §15-6. (§14/§16은 별도 세션에서 보리와 진행 중 — 이
> 갱신에서 다루지 않음.)
