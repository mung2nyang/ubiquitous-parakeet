...
**일일운행 2부(버그1 2중 스크롤 + 버그2 카드 이탈) 착수지시서 작성 완료,
보리 확인 대기 중.** `docs/report.md` §1 — 건드릴 파일 3개(`day-log.css`·
`CallDetailList.jsx`·`DayLogPage.jsx`), `[ ]` 미승인. 확인되면 작업자에게 전달.

- 원칙(보리 지시 2026-09-10): "화면 UI 대조 + 그 화면 전용 CSS를 `side-menu.css`에서
  분리"를 **한 슬라이스로 묶는다**(같은 화면을 두 번 열지 않으려고). 버그1·버그2도
  같은 이유로 한 슬라이스로 묶음(둘 다 일일운행 화면, 파일 3개로 §3 크기 안).

## 다음 할 일

1. ~~일일운행 2중 스크롤 버그 / 카드 이탈 버그~~ → 착수지시서 작성 완료,
   `docs/report.md` §1 보리 확인 대기(위 "지금 하는 일" 참고).
2. B그룹(즐겨찾기 칩·거래처 +추가 버튼·산재보험료 필드, `ui-comparison-report.md`
   §1-B) — 별도 슬라이스.
