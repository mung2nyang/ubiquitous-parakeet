...
**"원" 앞 공백 제거 착수지시서 `[ ]` 승인 대기(2026-09-15).**
`formatWon()` 전역 함수(14개 파일 공유)가 "100,000 원"(공백)을
반환 — 미수금 대조 중 발견. 원본도 확인해봤는데 같은 카드 안에서도
공백 유무가 들쭉날쭉해 원본과 맞추는 게 불가능 — 보리 결정으로
react-app은 공백 없이 통일. 상세 `docs/report.md`.
...
1. **`side-menu.css` 분리 슬라이스 G `[~]`**(위) — push·브라우저 검증·
   `[x]` 후 I/K/L 등 다음 덩어리(보리 확인). `docs/report.md`.
