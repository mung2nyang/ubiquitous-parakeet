...
> 최종 갱신: 2026-09-24 (이관 마무리 ② 5-5 계산서 모달 세무정보 + 거래처 "정보 작성 완료" 배지 `[x]` — react-app `550bca5`.
> 앞서 ① 11-7 `[x]`(`fc4ec83`). 우선순위 원칙을 "배포 목표"로 변경. 진행 중인 슬라이스 없음, 다음은 ③ 11-5·11-6.)
...
- **이관 마무리(보리 확정 2026-09-21, 순서는 AI 위임)** — `docs/roadmap.md` "이관 마무리 범위": ~~①11-7 산재보험 토글~~ `[x]`·~~②5-5 계산서 모달 세무정보 + 작성 완료 배지~~ `[x]`(2026-09-24, 아래 "완료")
  다음은 ③11-5 차량별 사업자정보 + 11-6 정산 계좌("기사 관리" 화면 하단) ④10-7 기사 본인 연동 해제(권한 확인 먼저).
...
1번(② 5-5) `[x]` 완료 — 지금은 2번(③ 11-5 사업자정보·11-6 정산 계좌).
