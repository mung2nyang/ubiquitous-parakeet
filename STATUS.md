...
> 최종 갱신: 2026-09-18 (§16 슬라이스 A(톱니바퀴 버튼+프로필 카드
> 이름/전화번호 표시) `[x]` push·CI 초록·보리 최종 승인까지 전부 완료.
> §15 슬라이스 E는 여전히 보류. §16 슬라이스 B는 다음 착수 대상)
...
§16 슬라이스 A(톱니바퀴 버튼+프로필 카드 이름/전화번호 표시) `[x]` 완료.
...
카드 이름/전화번호 표시, `[x]` 완료)/B(설정화면 뼈대)/C(차량별
설정값 실제 배선) 3슬라이스로 진행 중(보리 지시, 2026-09-18). **다음
착수 대상은 슬라이스 B** — 착수지시서는 보리 지정 시 작성. B·C 상세
설계는 `docs/roadmap.md` §16 준비 메모 참고(B는 `AppSettingsPage.jsx`
재사용 확정, C는 차량별 스코프 재설계).
