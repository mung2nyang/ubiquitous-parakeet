...
> 이동. 다음은 보리가 직접 진행할 "원본↔React UI 전수 대조" — 감시관 작업 아님,
> 기록만 해둠)
...
**[기록만 — 보리가 직접 진행, 감시관 작업 아님] 원본↔React UI 전수 대조** — 보리
지시(2026-09-07): 다음 건은 바닐라(`ubiquitous-parakeet`)와 react-app 화면을
사용자가 직접 전수 비교할 예정. 착수지시서·감시관 조사 불필요, 여기 기록만 해둠.
진행 방식·범위는 보리가 시작할 때 알려줄 것.
