...
> 최종 갱신: 2026-09-10 (**공용 헤더 스타일 통일(§1-2) ① CSS 슬라이스 —
> 구현·CI green·감시관 §5 7항목+브라우저 컴퓨티드 스타일 실측까지 완료,
> 보리 최종 브라우저 확인·승인 대기 중(`[~]`).** react-app `a351340`(보리
> push), `side-menu.css` 딱 1개 파일 25줄 삭제만(지시서와 완전 일치).
> 감시관이 `icon-btn`/`top-menu-btn`/`top-notification-btn` 3개 버튼을
> 직접 DOM에 렌더링해 `getComputedStyle` 대조 — 배경·테두리·모서리·크기
> 전부 일치 확인, hover 규칙도 보존 확인. 브라우저 실검증 가이드
> `docs/report.md` §1-2. 이전 작업(미연동 서브차량+사이드메뉴, §1~§16
> 전체 `[x]`)은 `docs/archive/sub-vehicle-management-and-side-menu.md`로 이관.)
>
...
**§1-2(공용 헤더 스타일 통일) — 5슬라이스 계획, ① CSS 통일 구현·검증
완료(`[~]`, 보리 브라우저 확인·승인 대기).** react-app `a351340`, CI green +
감시관 §5 7항목 + 컴퓨티드 스타일 실측 통과. 보리 확정: 슬라이스 분할
(①CSS 1파일 → ②~⑤ 화면 3~4개씩 묶어 햄버거 버튼 추가) + 기사연동관리는
연동·미연동 두 라우트 모두 적용. 상세·다음 슬라이스(②~⑤) 로드맵은
`docs/report.md` §2 참고.
