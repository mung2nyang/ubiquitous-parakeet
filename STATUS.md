...
> 최종 갱신: 2026-09-09 (**`main-calendar.css` 책임 분리 1~8차 전부 `[x]`.**
> 8차 콜상세 카드(call-detail-card.css)는 react-app `a731ca1`, CI green +
> 감시관 §5 7항목 통과 + 라이트/다크 컴퓨티드 스타일 13항목 전수 대조 완전
> 일치 확인 후 보리 명시 승인 **"승인/다음 진행해"**로 닫음. **남은 건
> 콜상세 폼(call-detail-form.css) `[x]`.** 보리 명시 승인 **"승인/다음
> 진행해"**로 §3 설계표의 컴포넌트별 슬라이스 전부 완료. **남은 건 공통/공유
> 규칙 재배치("해체") 1개뿐 — 재조사 완료 + 착수지시서 작성 완료
> (`docs/report.md` §24), 사용자 착수 승인 대기 중.** 재조사 중
> `.modal-title-stack`·`.autosave-status`가 실은 일지 전용인데 "공통" 구간에
> 남아있던 걸 발견해 `day-log-shell.css`로 보내는 예외 처리 포함. 상세
> `docs/report.md` §20~§24.)
...
**(보류) ④ 매출 탭 수치 불일치·거래처/미수금 데이터 차이 원인 조사** — 아직
착수 전 상태 그대로. 아래 "원본 vs react-app UI 전체 비교" 작업이 먼저.
...
- **10차(해체) 공통/공유 규칙 재배치 — 재조사·착수지시서 작성 완료, 승인 대기
  `[~]`.** `main-calendar.css`에 남은 마지막 230줄(전부 여러 화면 공동 소비)을
  신규 `app-shell-base.css`(27줄, 테마 변수·body·컨테이너)·`shared-controls.css`
  (약 187줄, 날짜 이동기·요약 카드·설정 헤더·아이콘 버튼·토글·입력 기본형)로
  나누고 `App.jsx`에 전역 import 2줄 추가, `CalendarPage.jsx`·`RevenuePage.jsx`·
  `LinkedDriverManagementPage.jsx`의 개별 import 제거. 재조사 중
  `.modal-title-stack`·`.autosave-status`(16줄)가 실제로는 일지 전용
  (`DayLogHeader`/`AutoSaveStatus` 각 1곳)인데 "공통" 구간에 잘못 남아있던 걸
  발견해 `day-log-shell.css`로 이동 예외 처리. `inlineSheetCss.test.js`가
  `main-calendar.css`를 직접 읽는 걸 발견해 파일 삭제 대신 빈 파일+주석 1줄로
  보존하기로 설계(테스트 코드는 안 건드림). 총 8파일 변경(기존 5+신규 2+스텁
  유지 1) — 지금까지 중 가장 큰 슬라이스라 사유를 명시해 승인 요청. 상세
  `docs/report.md` §24.
...
선택창·하단 네비·일지 정비/주유/기타·고정노선/파렛트·일지 셸·콜 목록·콜상세
카드·콜상세 폼 CSS 분리 9개 슬라이스 **전부 `[x]` 완료(§3 설계표 컴포넌트별
몫 전체 완료)** → **⑩ 해체(공통/공유 규칙 재배치) — 재조사·착수지시서 작성
완료(`docs/report.md` §24), 사용자 착수 승인 대기 중** → ⑪ 원본↔React UI
비교의 다음 화면 수정 재개. 기존 이관 계획 ④(매출 탭 수치 불일치·거래처/
미수금 데이터 차이 원인 조사)는 그 뒤까지 보류한다.
