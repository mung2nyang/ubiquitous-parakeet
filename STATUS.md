...
**콜상세 "거래처 +추가" 버튼 — `[x]` 최종 확정(2026-09-11, 보리 로그인
계정 브라우저 실검증 완료).** 200줄 주석 오차(`CallDetailForm.jsx`·
`DayLogPage.jsx`)는 보리가 직접 고치기로 함 — 후속 커밋 없이 종료. 상세
`docs/report.md`. **다음은 게스트 데이터 유실 버그 조사·착수(아래 "다음
할 일" 1번).**

## 다음 할 일

1. **게스트 데이터 유실 버그 조사·수정** [확인: 2026-09-11, 보리 재현 확인] —
   콜상세 저장 직후·새로고침 직후엔 `localStorage`(`reactPracticeWorkData:guest`)에
   정상 존재하지만, 그 뒤 몇 초~화면 전환이 지나면 조용히 빈 `{}`로 리셋됨.
   원인 추정: `hydrate.js`/`pendingWorkDataWrites.js`/`durableWriteGuard.js`
   등 기존 동기화·복구 계층이 게스트 모드를 다루는 방식.
2. **연동 기사 본인 로그의 거래처 스코프 불일치 조사·수정** [발견:
   2026-09-11, "거래처 +추가" 착수지시서 조사 중] — 기사 본인 계정 하이드레이트
   (`hydrateEmployedDriver.js`)가 차주 거래처 전체를 스코프 필터 없이
   내려받는데, 이 기사의 운행일지는 항상 `logId='main'`으로 취급돼
   `getClientsForLog`가 "스코프 없는(차주 일반) 거래처"만 보여준다 — 기사
   전용 거래처(`OwnerScopedClientsView.jsx` 기준)와 다른 집합일 수 있음.
3. "부가세 해제" 레이블 16px 잔존[범위 밖 발견] — 다음 관련 슬라이스에서 처리.
4. §1-A 이후 — 보리가 직접 작성한 §2~§14 화면별 대조 기록을 순차로. 각 화면 =
...
- **react-app**: `main` = `0db5bde`(콜상세 거래처 +추가 버튼, CI green). 클린.
- **ubiquitous-parakeet**: `main` = `4e58448`(일일운행 2부 `[x]` 확정) +
  이 세션의 문서 커밋(`STATUS.md`/`docs/report.md`/`docs/sot.md` — ③ 폐기
  + 거래처 +추가 착수·완료 기록). 클린.
- 확인 2026-09-11(감시관 세션, 작업자 커밋 확인 요청 처리). 정확한 HEAD·
  미커밋 범위는 매 세션 시작 시 `git log`/`git status`로 직접 재확인 (AGENTS §0-6).
