...
**고정노선 운송료가 정산액 계산에 안 잡히는 버그 `[~]`** — 착수 승인
후 코드·테스트 완료(`npm test` 172개·`tsc` 0에러). **브라우저 실검증
대기.** 상세는 `docs/report.md`.

## 다음 할 일

1. 보리 브라우저 실검증(이때 이전 슬라이스 `63ef902`의 "정산액 N%"
   표시도 같이 최종 확인)
2. 통과 시 커밋 → 보리 push(`63ef902` 포함) → CI 확인
