...
**`.toggle-btn` 알약 라운드 공용 버그 `[x]` 완료**(2026-09-16) —
`eba8eb1`, CI·Deploy 초록·보리 브라우저 실검증 완료(6곳 전부 사각
라운드로 정상 변경 확인). §9 슬라이스 1·2도 `[x]` 완료(`6ee827f`/
`03e1e86`). **§1~§8 UI 전부 `[x]` — §9~§14 UI 대조 + `side-menu.css`
분리 진행 중.** §4 남은 토글 닫기는 `InlineSheet` 리팩토링 뒤로 이월.

## 다음 할 일

1. §9 시각 결과(박스 배치·토글 §7 스타일 통일 등) 세부 재대조 후속
   슬라이스. 착수 전 범위 다시 논의.
...
- **react-app**: `main` = `eba8eb1`(`.toggle-btn` 라운드 버그 수정,
  origin 동일). §9 슬라이스 1·2 + 라운드 버그 전부 커밋·푸시·CI
  초록·보리 실검증 완료.
