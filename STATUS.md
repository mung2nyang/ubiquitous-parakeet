...
> 연결 ④정비/주유/기타 연결) 중 **①번 착수지시서 작성 완료, 사용자 착수 승인
> 대기 중**(`docs/report.md`). 도메인 개념 정의는 `docs/sot.md` §0에 새로 기록.
...
①(관리 진입점 신설) 착수지시서 작성 완료, 사용자 착수 승인 대기 중.
...
- **4단계 계획**: ①진입점 신설(연동 기사 관리 화면·메뉴 재사용) → ②거래처 태깅
  버그 수정 → ③관리 화면에 거래처 연결 → ④관리 화면에 정비/주유/기타 연결. **①번
  착수지시서 작성 완료**(`docs/report.md`) — 기존 `LinkedDriverManagementPage.jsx`의
  정산 계산 함수들이 `link=null`을 이미 안전하게 받아들이는 것 확인(수정 불필요),
  모드 분기는 신규 `domain/driverManagementContext.js`로 분리설계. 사용자 착수 승인
  대기 중.
