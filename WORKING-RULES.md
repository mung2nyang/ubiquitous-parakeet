...
- 실패·에러가 발생하면 코드를 쓰기 전에 사용자에게 먼저 물어라.
...
- 완료 판정 금지 규칙:
  - 요구사항 미충족 항목을 “알려진 한계”, “후속 Step 범위”, “의도적 제외”로 기록하며 완료 처리 금지.
  - 미충족 항목이 1개라도 있으면 [x] 표시, 완료/100% 보고, git commit, 다음 Step 착수를 전면 금지한다. (범위 조정은 사전 사용자 승인 필수)
...
- 절차: 교차검증 결과를 한국어로 상세 보고 → 사용자 승인 획득 → 한국어 커밋 메시지로 커밋 실행.
...
- UI 및 상태 직접 검출력:
  - 테스트 이름이 주장하는 Store/localStorage/journal/API/UI 상태를 Assert가 직접 검사해야 한다.
  - 실제 UI 호출부 안전성을 순수 함수 테스트만으로 완료 주장하지 마라. React 이벤트 핸들러부터 최종 상태까지의 컴포넌트 또는 통합 테스트를 유지하라.
- Revert-and-confirm-fail: 신규 회귀 테스트는 프로덕션 결함을 임시 복원했을 때 실제로 FAIL하는지 확인 후 원복.
