...
> `AGENTS.md` 작업 원칙의 제정·수정·삭제 권한은 오직 사용자(보리)에게만 있다.
> 프로젝트 내 모든 공식 장부(docs/audit.md, docs/sot.md 등)의 기록·작성·갱신은 오직 감시관(Auditor)만 수행한다. 작업자(Cursor, Claude Code CLI 등)는 임의로 .md 파일을 수정하거나 덧붙여 쓸 수 없다.
>모든 작업은 반드시 ① 작업 계획 수립 시(착수 전), ② 공식 착수 시, ③ 작업 완료 및 검증 시의 3시점에 빠짐없이 기존 장부(`docs/audit.md`·`docs/sot.m`등)에 감시관(Auditor)이 직접 기록을 남겨야 하며, 기록 없는 작업 착수는 전면 금지한다.
...
> 보리의 명시적 승인으로 추가된 원칙이다. 이 절은 이후 감사 FAIL·리팩토링에서
...
- 절차: 교차검증 결과를 한국어로 상세 보고  → 사용자의 승인 획득 → 감시관 검증 및 승인 완료 후 작업자가 한국어 커밋 메시지 커밋 실행.
- 푸시 권한 통제: 푸시는 사용자가 최종 커밋 내역을 검토한 뒤 반드시 사용자(보리)가 직접 수행한다. 에이전트의 임의 push 및 push 대행을 전면 금지한다.
- 저장소 간 동기화 규칙: 보고·원칙 저장소(`ubiquitous-parakeet`)와 구현 저장소(`react-app`)의 작업 단계 및 커밋 상태는 항상 상호 일치해야 한다.
