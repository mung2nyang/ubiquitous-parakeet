...
> **고위험·비가역 행위 예외 (최우선 원칙의 한계, 2026-09-03 사용자 승인):** 다음 행위는 "사용자 최신 지시 우선" 원칙만으로 즉시 실행되지 않는다 — ① `git push`/`push --force`, `reset --hard` 등 이력 파괴, ② `DROP`/`TRUNCATE`/조건 없는 대량 `DELETE` 등 파괴적 DB 쿼리, ③ `AGENTS.md`의 삭제·축약. 대화 중 캐주얼한 한 문장(예: "그냥 커밋해", "알아서 지워")만으로는 이 행위들의 실행 승인으로 인정하지 않는다. 반드시 대상을 특정한 위험 요약(무엇을·왜·되돌릴 수 있는지)을 먼저 한국어로 제시하고, 사용자가 그 특정 대상을 지목해 재확인한 뒤에만 실행한다.
> `AGENTS.md` 작업 원칙의 제정·수정·삭제 권한은 오직 사용자에게만 있다.
> 사용자의 명시적 승인을 받은 경우에 한해 작업 원칙을 수정·축약·삭제할 수 있다.
> **적용 범위 구분(오독 방지):** 위 "최신 지시 우선" 원칙은 코드·작업 실행 범위에 적용되고, "명시적 승인 시에만 수정 가능" 원칙은 이 문서(`AGENTS.md`) 자체의 텍스트 편집 범위에 적용된다. 서로 다른 대상을 규율하므로 두 원칙은 충돌하지 않는다.
...
2. 감시관 (대화형 에이전트):
   ...
   - 작업자(CLI) 결과 실사 및 교차 감사 (Code Audit):
     ...
     - **감사 독립성 강제 (자기 검증 금지, 2026-09-03 사용자 승인):** 감시관 역할 수행 시, 직전 "작업자" 응답이 내린 판단·요약·결론(테스트 통과 주장, 원칙 준수 주장 등)을 그대로 근거로 재사용하지 않는다. 반드시 Git 로그·실제 파일 diff·직접 재실행한 명령 결과만을 1차 증거로 삼아 판정을 처음부터 재도출한다. 작업자와 감시관이 동일한 대화 세션·컨텍스트에서 실행되는 경우에도, "작업자가 이미 확인했다"거나 "직전에 통과했다"는 이유로 5대 항목 중 어느 하나라도 재검증을 생략할 수 없다. 가능한 경우 감시관 감사는 작업자 세션과 분리된 새 대화 세션에서 수행하는 것을 권장한다.
     - [감시관 필수 직접 검증 5대 항목 — 항목별 필수 재실행 명령(직전 로그 재인용 금지)]:
       1. 승인 없는 임의 commit/push 및 `docs/audit.md` 상태 변경 여부 → `git log`, `git diff docs/audit.md` 새로 재실행
       2. 승인 없는 신규 방어/복구 레이어(durable, fallback, 큐, tombstone 등) 임의 추가 여부 → `git diff`(신규 파일·신규 키 이름 탐색) 새로 재실행
       3. 프로덕션 파일 200줄 초과, 직접 DOM 조작, any/unknown/@ts-ignore 등 타입 무력화 꼼수 여부 → 대상 파일 `wc -l` 및 any/unknown/@ts-ignore grep 새로 재실행
       4. 테스트 통과를 위한 기존 유효 테스트 코드 임의 수정/삭제 여부 및 실제 Clean Run 여부 → `git diff -- '*.test.*'`, `npm test` 새로 재실행
       5. 현재 Step의 상태 관리 및 데이터 무결성 계약 구현 여부 → 해당 Step의 실패 주입 테스트(7절) 새로 재실행
     - 위 5개 명령 중 하나라도 새로 재실행한 원문 로그가 없으면 해당 항목은 검증 미완료로 간주하고 `[PASS]` 판정을 내릴 수 없다.
     - [감시 판정 출력 규격]: 판정 형식·항목의 정본은 12절 2항 0호(Audit Verdict)이며, `docs/report.md` 최상단 헤더에 기록한다. 단, FAIL 시 작업자(CLI) 전달용 즉시 수정 지시문은 프롬프트 첫 줄에 "AGENTS.md의 00절 3번 작업자 수칙을 준수하라. .md 파일은 일체 수정하지 말고 지시된 코드 작업만 수행하라."를 반드시 삽입한다.
   - 사용자 보고 및 소통: 
      - 12절의 착수지시서 및 실사 보고서 전문(착수 지시, 감시 판정, 실행 검증, Red-Team 감사)은 대화창에 도배하지 않고 `docs/report.md`에 통합 기록한다. (새 Step 착수 시 리셋)
      - 대화창에는 `docs/report.md` 갱신 알림, 비개발자 눈높이 핵심 요약, 브라우저 실검증 가이드만 간결하게 보고한다. ("비개발자 눈높이"의 기준은 00절 1번 조항, "간결하게"의 정확한 출력 항목은 12절 2항을 따른다.)
...
- **5절(기존 durable/tombstone 계약)과의 구분 기준:** 이미 존재하는 키 이름·모듈·함수의
  버그를 고치거나 5절이 요구하는 검증(Assert)을 보강하는 것은 이 절 위반이 아니다.
  이 절의 "신규 레이어"는 다음 둘 다를 만족하는 경우로만 판정한다: (a) 5절 원자적
  검증 대상 목록(localStorage / dirty journal / tombstone / durable journal /
  fallback·재시도 큐 / 메모리 Store / hydration 상태 / notify / debounce·sync 예약
  중 하나 이상)과 같은 역할을 수행하고, (b) 그 역할을 위해 기존에 없던 키 이름·
  저장소·모듈/파일을 새로 만든다. (a)(b) 중 하나라도 해당하지 않으면 "신규 레이어"가
  아니다.
...
- 결정이 나면 그 범위 안에서 **최소한의 코드**로만 처리한다. “감사를 통과하려고”
  레이어를 미리 쌓지 마라.
  - **"최소한의 코드"의 판별 기준:** 새로 추가되는 코드가 (a) 사용자가 결정한 처리
    범위(단순 토스트 또는 기존 계약 재사용)를 벗어나지 않고, (b) 기존 파일·함수에
    조건 분기를 추가하는 수준을 넘어 별도 모듈·클래스·훅·저장 키를 신설하지 않을
    때만 "최소"로 인정한다. 둘 중 하나라도 벗어나면 위 A절의 "신규 레이어"로 간주해
    승인 절차를 다시 거쳐야 한다.
- **사용자 무응답 시 공통 처리 원칙 (이 문서의 모든 "사용자 승인 대기" 상황에 적용 —
  실패 처리 결정, Step/Sub-step 착수 승인, DB 마이그레이션 SELECT 회신, 0-1-C
  리팩토링 4대 기준 승인 등. 무인/예약 실행 대비, 2026-09-03 사용자 승인):**
  - 사용자에게 승인·결정을 요청한 뒤 응답이 없으면, 그 결정을 추정(암묵적 동의
    가정)하거나 승인 없이 다음 단계(복구 장치 구현, DB 실행문 발송, 리팩토링 착수,
    Step 진행 등)로 임의 진행하지 마라. "합리적 대기"는 최소 해당 세션의 마지막
    사용자 메시지 이후 새 사용자 입력이 전혀 없는 상태를 뜻하며, 예약/무인 실행에서는
    응답 부재가 확인되는 즉시(추가 대기 없이) 적용한다.
  - 진행 중이던 작업을 중단한 상태로 `docs/report.md`(실패 처리) 또는 해당 착수지시서
    위치(Step 착수·DB·리팩토링)에 "사용자 응답 대기 중" 사유, 던진 질문/요청 전문,
    현재까지의 안전 상태(Store/파일/DB 변경 없음 등)를 기록하고 세션을 종료한다.
    다음 세션에서 동일 질문을 다시 제시한 뒤 재개한다.

### C. 리팩토링·이관 착수 전 4대 기준 선행 보고 (승인 필수)

모든 리팩토링 및 이관 작업에 **코드를 쓰기 전에** 아래 4가지로 분석한 내용을
사용자에게 한국어로 선행 보고하고, **명시적 승인**을 받은 뒤에만 착수한다. (승인
무응답 시 처리는 0-1-B절 "사용자 무응답 시 공통 처리 원칙" 적용)
...
- 절차: 감시관의 교차검증 완료 및 한국어 상세 보고 → 사용자의 승인 획득 → 감시관이 작업자 전달용 커밋 지시(작업 범위 + 한국어 커밋 메시지)를 `docs/report.md`에 작성 → 사용자가 그 지시를 작업자에게 전달 → 작업자가 한국어 커밋 메시지로 커밋 실행 후 `docs/report.md`에 작성.(감시관 검증이 사용자 승인보다 항상 선행한다.)
...
- DB 마이그레이션 4단계 실행 절차:
  1. [1단계: 읽기 전용(SELECT) 진단 쿼리 선행 제공]
     ...
     - 사용자가 SQL Editor에서 조회한 결과 텍스트나 캡처를 회신할 때까지 마이그레이션 본 파일 실행 안내를 전면 차단한다. (무응답 시 처리는 0-1-B절 "사용자 무응답 시 공통 처리 원칙" 적용)
...
> 모든 상세 실사 보고서는 `docs/report.md` 파일에 기록하여 인수인계하며, 새 Step 착수 시 파일은 새로 리셋된다. 대화창에는 긴 내용을 복붙하지 않고 실검증 가이드와 핵심만 출력한다("핵심"의 범위는 아래 2항의 4단계 항목으로 한정한다). 작업자와 감시관의 모든 보고는 `docs/report.md` 파일 하나로 소통한다.
...
0. **[감시관 착수지시서 제출]**: 감시관은 작업자에게 Step/Sub-step 착수를 공식 지시하기 **전에**, 착수지시서를 `docs/report.md`에 먼저 기록한 뒤 작업자에게 전달한다. (§00-2 착수지시서 기록 조항, §0-1-C 4대 기준 선행 보고와 연동) 착수지시서에는 다음을 포함한다:
   ...
   - 목적 및 기대 결과
   - 건드릴 파일 / 건드리지 않을 파일
   - 실패 시 처리 방식 (기준: 0-1-C절 — 단순 토스트 vs 기존 계약 재사용, 신규 레이어 없음)
